...
const ignores = fs
	.readFileSync(path.join(import.meta.dirname, ".eslint-ignore"), "utf8")
	.toString()
	.split(/\r\n|\n/)
...
export default tseslint.config(
	...
	// All files (JS and TS)
	{
		...
		rules: {
			...
			"header/header": [
				...
				[
					{ pattern: "---------------------------------------------------------------------------------------------" },
					{ pattern: " \\*  Copyright \\(c\\) (Microsoft Corporation|Nyrve contributors)\\. All rights reserved\\." },
					" *  Licensed under the MIT License. See License.txt in the project root for license information.",
					{ pattern: " \\*--------------------------------------------------------------------------------------------" },
				],
			],
	...
	{
		files: ["extensions/git/src/**/*.ts"],
		ignores: ["extensions/git/src/api/git.constants.ts"],
		languageOptions: {
			parser: tseslint.parser,
		},
		plugins: {
					...
					patterns: [
						{
							...
							message:
								"Use 'import type' for types from git.d.ts and import runtime const enum values from git.constants instead",
						},
					],
				},
			],
		...
		rules: {
			...
			"no-restricted-syntax": [
				...
				},
				{
					...
					message:
						"querySelector should not be used as relying on selectors is very fragile. Use dom.ts h() to build your elements and access them directly.",
				},
				{
					selector: "CallExpression[callee.property.name='querySelectorAll']",
					message:
						"querySelectorAll should not be used as relying on selectors is very fragile. Use dom.ts h() to build your elements and access them directly.",
				},
				{
					selector: "CallExpression[callee.property.name='getElementById']",
					message:
						"getElementById should not be used as relying on selectors is very fragile. Use dom.ts h() to build your elements and access them directly.",
				},
				{
					selector:
						"CallExpression[callee.property.name='getElementsByClassName']",
					message:
						"getElementsByClassName should not be used as relying on selectors is very fragile. Use dom.ts h() to build your elements and access them directly.",
				},
				{
					selector:
						"CallExpression[callee.property.name='getElementsByTagName']",
					message:
						"getElementsByTagName should not be used as relying on selectors is very fragile. Use dom.ts h() to build your elements and access them directly.",
				},
				{
					selector: "CallExpression[callee.property.name='getElementsByName']",
					message:
						"getElementsByName should not be used as relying on selectors is very fragile. Use dom.ts h() to build your elements and access them directly.",
				},
				{
					selector:
						"CallExpression[callee.property.name='getElementsByTagNameNS']",
					message:
						"getElementsByTagNameNS should not be used as relying on selectors is very fragile. Use dom.ts h() to build your elements and access them directly.",
				},
			],
			"no-restricted-globals": [
				"warn",
		...
		rules: {
			...
			"no-restricted-syntax": [
				"warn",
				{
					...
					message:
						"Use static imports instead of dynamic require() calls to enable tree-shaking.",
				},
				{
					selector: `ImportExpression[source.type!='Literal']`,
					message:
						"Use static imports instead of dynamic import() calls to enable tree-shaking.",
				},
			],
		},
	},
	// markdown-language-features
	{
		...
		rules: {
			"no-restricted-syntax": [
				...
				{
					selector:
						':matches(PropertyDefinition, TSParameterProperty, MethodDefinition[key.name!="constructor"])[accessibility="private"]',
