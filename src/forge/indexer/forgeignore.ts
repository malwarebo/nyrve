...
export const IForgeIgnoreService = createDecorator<IForgeIgnoreService>('forgeIgnoreService');

export interface IForgeIgnoreService {
	readonly _serviceBrand: undefined;

	/** Check if a file path should be excluded from indexing. */
	isIgnored(filePath: string): boolean;

	/** Reload ignore patterns from .forgeignore and .gitignore files. */
	reload(): Promise<void>;
...
export class ForgeIgnoreService extends Disposable implements IForgeIgnoreService {
	...
	async reload(): Promise<void> {
		...
		// Load .forgeignore (overrides)
		const forgeignorePatterns = await this._loadIgnoreFile(URI.joinPath(projectRoot, '.forgeignore'));
		patterns.push(...forgeignorePatterns);

		this._patterns = patterns;
		this._compilePatterns();

		this.logService.info(`[Forge] Loaded ${patterns.length} ignore patterns`);
	}
	...
	exceedsMaxFileSize(fileSizeBytes: number): boolean {
		const maxSize = this.configurationService.getValue<number>('forge.indexer.maxFileSize') ?? 1048576;
...
registerSingleton(IForgeIgnoreService, ForgeIgnoreService, InstantiationType.Delayed);
