...
export const IForgeDecisionJournal = createDecorator<IForgeDecisionJournal>('forgeDecisionJournal');

export interface IForgeDecisionJournal {
...
export class ForgeDecisionJournal extends Disposable implements IForgeDecisionJournal {
	...
	/** In-memory store. Persisted to .forge/decisions.json. */
	...
	async addDecision(partial: Partial<DecisionEntry>): Promise<string> {
		...
		const maxEntries = this.configurationService.getValue<number>('forge.memory.decisions.maxEntries') ?? 500;
		...
		this.logService.info(`[Forge] Decision added: "${entry.title}" (${entry.source})`);
	...
	private async _loadFromDisk(): Promise<void> {
		...
		try {
			const uri = URI.joinPath(root, '.forge', 'decisions.json');
			const content = await this.fileService.readFile(uri);
			this._entries = JSON.parse(content.value.toString());
			this.logService.info(`[Forge] Loaded ${this._entries.length} decisions from disk`);
		} catch {
	...
	private async _saveToDisk(): Promise<void> {
		...
		try {
			...
			this.logService.error(`[Forge] Failed to save decisions: ${error}`);
		}
	}
...
registerSingleton(IForgeDecisionJournal, ForgeDecisionJournal, InstantiationType.Delayed);
