...
import { IForgeAgentEngine } from '../agent/agent-engine.js';
import { IForgeModelRouter } from '../agent/model-router.js';
import { ForgeConfigScanner, TechStackEntry } from './dna/config-scanner.js';
import { ForgeStructureScanner, ModuleInfo, DependencyEdge } from './dna/structure-scanner.js';
import { ForgeGitAnalyzer, FileHotspot, FileCoupling } from './dna/git-analyzer.js';
import { ForgeComplexityAnalyzer, TechDebtItem } from './dna/complexity-analyzer.js';
import { ForgePatternDetector, CodePattern, Convention } from './dna/pattern-detector.js';
...
export const IForgeProjectDNA = createDecorator<IForgeProjectDNA>('forgeProjectDNA');

export interface IForgeProjectDNA {
...
export class ForgeProjectDNAService extends Disposable implements IForgeProjectDNA {
	...
	private readonly configScanner: ForgeConfigScanner;
	private readonly structureScanner: ForgeStructureScanner;
	private readonly gitAnalyzer: ForgeGitAnalyzer;
	private readonly complexityAnalyzer: ForgeComplexityAnalyzer;
	private readonly patternDetector: ForgePatternDetector;

	constructor(
		@IFileService private readonly fileService: IFileService,
		@IForgeIndexManager indexManager: IForgeIndexManager,
		@IForgeAgentEngine agentEngine: IForgeAgentEngine,
		@IForgeModelRouter modelRouter: IForgeModelRouter,
		...
		this.configScanner = new ForgeConfigScanner(fileService, logService);
		this.structureScanner = new ForgeStructureScanner(indexManager, logService);
		this.gitAnalyzer = new ForgeGitAnalyzer(workspaceContextService, logService);
		this.complexityAnalyzer = new ForgeComplexityAnalyzer(indexManager, workspaceContextService, logService);
		this.patternDetector = new ForgePatternDetector(agentEngine, modelRouter, indexManager, logService);
	...
	async fullScan(): Promise<ProjectDNA> {
		const enabled = this.configurationService.getValue<boolean>('forge.memory.dna.enabled') ?? true;
		if (!enabled) {
			this.logService.info('[Forge] Project DNA scanning is disabled');
			return this._emptyDNA();
		}

		const startTime = Date.now();
		this.logService.info('[Forge] Starting full DNA scan...');
		...
		const historyDays = this.configurationService.getValue<number>('forge.memory.dna.gitHistoryDays') ?? 90;
		...
		try {
			...
			this.logService.info(`[Forge] Full DNA scan complete in ${scanDuration}ms`);
			this._onDidScanComplete.fire(dna);

			return dna;
		} catch (error) {
			this.logService.error(`[Forge] DNA scan failed: ${error}`);
			return this._emptyDNA();
		}
	}
	...
	async incrementalUpdate(filePath: string): Promise<void> {
		...
		const enabled = this.configurationService.getValue<boolean>('forge.memory.dna.incrementalUpdates') ?? true;
		if (!enabled) {
			return;
		}

		this.logService.trace(`[Forge] DNA incremental update for: ${filePath}`);
	...
	private async _saveToDisk(dna: ProjectDNA): Promise<void> {
		...
		try {
			const uri = URI.joinPath(root, '.forge', 'project-dna.json');
			const content = JSON.stringify(dna, null, 2);
			await this.fileService.writeFile(uri, VSBuffer.fromString(content));
		} catch (error) {
			this.logService.error(`[Forge] Failed to save DNA: ${error}`);
		}
	}
	...
	private async _loadFromDisk(): Promise<void> {
		...
		try {
			...
			if (this._dna) {
				this._compressedSummary = compressDNA(this._dna);
				this.logService.info(`[Forge] Loaded DNA from disk (last scan: ${this._dna.lastFullScan})`);
			}
		} catch {
...
registerSingleton(IForgeProjectDNA, ForgeProjectDNAService, InstantiationType.Delayed);
