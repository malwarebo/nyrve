...
import { IForgeProjectDNA } from './project-dna.js';
import { IForgeDecisionJournal } from './decision-journal.js';
import { IForgeTeamKnowledge } from './team-knowledge.js';
...
export const IForgeMemoryRetriever = createDecorator<IForgeMemoryRetriever>('forgeMemoryRetriever');

export interface IForgeMemoryRetriever {
...
export class ForgeMemoryRetriever extends Disposable implements IForgeMemoryRetriever {
	declare readonly _serviceBrand: undefined;

	constructor(
		@IForgeProjectDNA private readonly projectDNA: IForgeProjectDNA,
		@IForgeDecisionJournal private readonly decisionJournal: IForgeDecisionJournal,
		@IForgeTeamKnowledge private readonly teamKnowledge: IForgeTeamKnowledge,
	...
	async retrieve(query: string, options?: RetrievalOptions): Promise<MemoryContext> {
		...
		const maxTokens = options?.maxTokens
			?? this.configurationService.getValue<number>('forge.memory.retrieval.maxTokens')
			?? 3000;
		const layers = options?.layers ?? ['team', 'dna', 'decisions'];

		const deepMemoryEnabled = this.configurationService.getValue<boolean>('forge.memory.deepMemoryEnabled') ?? true;
		...
		this.logService.trace(
			`[Forge] Memory retrieved: ${totalTokens} tokens in ${retrievalTime}ms ` +
...
registerSingleton(IForgeMemoryRetriever, ForgeMemoryRetriever, InstantiationType.Delayed);
