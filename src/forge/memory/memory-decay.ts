...
import { IForgeMemoryEngine } from './memory-engine.js';

// --- Service Interface ---

export const IForgeMemoryDecay = createDecorator<IForgeMemoryDecay>('forgeMemoryDecay');

export interface IForgeMemoryDecay {
...
export class ForgeMemoryDecay extends Disposable implements IForgeMemoryDecay {
	...
	runDecay(): void {
		const decayDays = this.configurationService.getValue<number>('forge.memory.decayDays') ?? 90;
		const now = Date.now();
		...
		if (decayedCount > 0) {
			this.logService.info(`[Forge] Decayed ${decayedCount} stale memories`);
		}
	}
	...
	archiveStale(): number {
		...
		if (archivedCount > 0) {
			this.logService.info(`[Forge] Archived ${archivedCount} low-confidence memories`);
		}
	...
	compact(): number {
		...
		if (mergedCount > 0) {
			this.logService.info(`[Forge] Compacted ${mergedCount} similar memories`);
		}
...
registerSingleton(IForgeMemoryDecay, ForgeMemoryDecay, InstantiationType.Delayed);
