...
export const IForgeMemoryEngine = createDecorator<IForgeMemoryEngine>('forgeMemoryEngine');

export interface IForgeMemoryEngine {
...
export class ForgeMemoryEngine extends Disposable implements IForgeMemoryEngine {
	...
	async load(): Promise<void> {
		...
		try {
			...
			this.logService.info(`[Forge] Loaded ${this._memories.size} memories`);
		} catch (e) {
			this.logService.warn(`[Forge] Failed to load memories: ${e}`);
		}
	}
	...
	async save(): Promise<void> {
		...
		try {
			...
			this.logService.trace(`[Forge] Saved ${entries.length} memories`);
		} catch (e) {
			this.logService.warn(`[Forge] Failed to save memories: ${e}`);
		}
	}

	addMemory(entry: Omit<MemoryEntry, 'id' | 'createdAt' | 'lastAccessedAt' | 'accessCount'>): MemoryEntry {
		const maxEntries = this.configurationService.getValue<number>('forge.memory.maxEntries') ?? 1000;
		...
		this.logService.trace(`[Forge] Added memory: ${memory.type} — ${memory.content.slice(0, 60)}...`);
	...
	private _getStorageUri(): URI | undefined {
		...
		return URI.joinPath(projectRoot, '.forge', 'memory.json');
	}
}

registerSingleton(IForgeMemoryEngine, ForgeMemoryEngine, InstantiationType.Delayed);
