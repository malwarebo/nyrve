...
export const IForgeTeamKnowledge = createDecorator<IForgeTeamKnowledge>('forgeTeamKnowledge');

export interface IForgeTeamKnowledge {
...
export class ForgeTeamKnowledgeService extends Disposable implements IForgeTeamKnowledge {
	...
	getFilePath(): string {
		return this.configurationService.getValue<string>('forge.memory.team.filePath') ?? '.forge/team-knowledge.md';
	}
	...
	async suggestAddition(
		...
		const enabled = this.configurationService.getValue<boolean>('forge.memory.team.suggestAdditions') ?? true;
		if (!enabled) {
			return;
		}
		...
		this.logService.info(`[Forge] Team knowledge suggestion: "${content.slice(0, 50)}..." for section "${section}"`);
	}
	...
	async approveSuggestion(id: string): Promise<void> {
		...
		this.logService.info(`[Forge] Team knowledge suggestion approved: "${suggestion.content.slice(0, 50)}..."`);
	}
	...
	private async _writeFile(content: string): Promise<void> {
		...
		try {
			...
			this.logService.error(`[Forge] Failed to write team knowledge: ${error}`);
		}
	}
	...
	private async _loadSuggestions(): Promise<void> {
		...
		try {
			const uri = URI.joinPath(root, '.forge', 'pending-suggestions.json');
			const content = await this.fileService.readFile(uri);
	...
	private async _saveSuggestions(): Promise<void> {
		...
		try {
			...
			this.logService.error(`[Forge] Failed to save suggestions: ${error}`);
		}
	}
...
registerSingleton(IForgeTeamKnowledge, ForgeTeamKnowledgeService, InstantiationType.Delayed);
