...
import { MemorySource, MemoryType, IForgeMemoryEngine } from './memory-engine.js';
...
export const IForgeMemoryExtractor = createDecorator<IForgeMemoryExtractor>('forgeMemoryExtractor');

export interface IForgeMemoryExtractor {
...
export class ForgeMemoryExtractor extends Disposable implements IForgeMemoryExtractor {
	...
	extractFromConversation(messages: readonly ConversationMessage[]): void {
		const autoExtract = this.configurationService.getValue<boolean>('forge.memory.autoExtract') ?? true;
		if (!autoExtract) {
			return;
		}
	...
	processExplicitMemory(content: string): void {
		...
		this.logService.info(`[Forge] Stored explicit memory: ${content.slice(0, 60)}...`);
	}
...
registerSingleton(IForgeMemoryExtractor, ForgeMemoryExtractor, InstantiationType.Delayed);
