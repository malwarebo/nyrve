...
export class ForgePatternDetector {
	constructor(
		private readonly agentEngine: IForgeAgentEngine,
		private readonly modelRouter: IForgeModelRouter,
		private readonly indexManager: IForgeIndexManager,
		private readonly logService: ILogService,
	) {}

	async detect(): Promise<PatternDetectionResult> {
		this.logService.info('[Forge] Pattern detector: sampling files for pattern analysis...');
		...
		try {
			...
			this.logService.error(`[Forge] Pattern detection failed: ${error}`);
			return { patterns: [], conventions: [] };
		}
	}
	...
	private _parseResponse(response: string): PatternDetectionResult {
		try {
			...
			this.logService.info(`[Forge] Pattern detection: found ${patterns.length} patterns, ${conventions.length} conventions`);

			return { patterns, conventions };
		} catch (error) {
			this.logService.error(`[Forge] Failed to parse pattern detection response: ${error}`);
			return { patterns: [], conventions: [] };
		}
	}
}
