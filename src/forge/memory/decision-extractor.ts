...
import { ForgeMessage, IForgeAgentEngine } from '../agent/agent-engine.js';
import { IForgeModelRouter } from '../agent/model-router.js';
import { IForgeDecisionJournal, DecisionEntry } from './decision-journal.js';

// --- Service Interface ---

export const IForgeDecisionExtractor = createDecorator<IForgeDecisionExtractor>('forgeDecisionExtractor');

export interface IForgeDecisionExtractor {
	readonly _serviceBrand: undefined;

	/** Extract decisions from a conversation. */
	extractFromConversation(messages: ForgeMessage[], conversationId?: string): Promise<DecisionEntry[]>;
...
export class ForgeDecisionExtractor extends Disposable implements IForgeDecisionExtractor {
	...
	constructor(
		...
		@IForgeDecisionJournal private readonly journal: IForgeDecisionJournal,
	...
	async extractFromConversation(messages: ForgeMessage[], conversationId?: string): Promise<DecisionEntry[]> {
		const enabled = this.configurationService.getValue<boolean>('forge.memory.decisions.autoExtract') ?? true;
		if (!enabled) {
			return [];
		}
		...
		this.logService.info('[Forge] Extracting decisions from conversation...');
		...
		try {
			...
			if (decisions.length > 0) {
				this.logService.info(`[Forge] Extracted ${decisions.length} decision(s) from conversation`);
			}

			return decisions;
		} catch (error) {
			this.logService.error(`[Forge] Decision extraction failed: ${error}`);
			return [];
		}
	}

	async extractFromCommit(commitHash: string, message: string, diff: string): Promise<DecisionEntry[]> {
		const enabled = this.configurationService.getValue<boolean>('forge.memory.decisions.extractFromCommits') ?? true;
		if (!enabled) {
			return [];
		}
		...
		this.logService.info(`[Forge] Extracting decisions from commit ${commitHash.slice(0, 7)}...`);
		...
		try {
			...
			this.logService.error(`[Forge] Commit decision extraction failed: ${error}`);
			return [];
		}
	}
...
${conversation.slice(0, 10000)}`;
	...
	private _parseExtractionResponse(
		...
		try {
			...
			this.logService.error(`[Forge] Failed to parse extraction response: ${error}`);
			return [];
		}
	}
}

registerSingleton(IForgeDecisionExtractor, ForgeDecisionExtractor, InstantiationType.Delayed);
