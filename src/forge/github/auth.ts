...
export const IForgeGitHubAuth = createDecorator<IForgeGitHubAuth>('forgeGitHubAuth');

export interface IForgeGitHubAuth {
...
export class ForgeGitHubAuth extends Disposable implements IForgeGitHubAuth {
	...
	async getAuthenticatedUser(): Promise<{ login: string; name: string | null } | undefined> {
		...
		try {
			...
			if (!response.ok) {
				this.logService.warn(`[Forge] GitHub auth check failed: ${response.status}`);
				return undefined;
			}
			...
			this.logService.warn(`[Forge] GitHub auth check error: ${e}`);
			return undefined;
		}
	}
}

registerSingleton(IForgeGitHubAuth, ForgeGitHubAuth, InstantiationType.Delayed);
