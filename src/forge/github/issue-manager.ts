...
import { GitHubIssue, IForgeGitHubService } from './github-service.js';

// --- Service Interface ---

export const IForgeIssueManager = createDecorator<IForgeIssueManager>('forgeIssueManager');

export interface IForgeIssueManager {
...
export class ForgeIssueManager extends Disposable implements IForgeIssueManager {
	...
	async fetchIssues(owner: string, repo: string): Promise<readonly GitHubIssue[]> {
		...
		this.logService.trace(`[Forge] Fetched ${issues.length} issues from ${owner}/${repo}`);
		return issues;
	}
...
registerSingleton(IForgeIssueManager, ForgeIssueManager, InstantiationType.Delayed);
