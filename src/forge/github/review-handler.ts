...
import { IForgeGitHubService } from './github-service.js';
...
export const IForgeReviewHandler = createDecorator<IForgeReviewHandler>('forgeReviewHandler');

export interface IForgeReviewHandler {
...
export class ForgeReviewHandler extends Disposable implements IForgeReviewHandler {
	...
	async postReply(owner: string, repo: string, prNumber: number, body: string, inReplyTo?: number): Promise<void> {
		...
		this.logService.info(`[Forge] Posted review reply on PR #${prNumber}`);
	}
	...
	startPolling(owner: string, repo: string, prNumber: number, intervalMs: number = 60000): void {
		...
		this._pollTimer = setInterval(async () => {
			try {
				...
				this.logService.warn(`[Forge] Comment polling error: ${e}`);
			}
		}, intervalMs);

		this.logService.trace(`[Forge] Started polling comments for PR #${prNumber}`);
	}
...
registerSingleton(IForgeReviewHandler, ForgeReviewHandler, InstantiationType.Delayed);
