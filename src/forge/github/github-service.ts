...
import { IForgeGitHubAuth } from './auth.js';
...
export const IForgeGitHubService = createDecorator<IForgeGitHubService>('forgeGitHubService');

export interface IForgeGitHubService {
...
export class ForgeGitHubService extends Disposable implements IForgeGitHubService {
	declare readonly _serviceBrand: undefined;

	private readonly _apiBase = 'https://api.github.com';

	constructor(
		@IForgeGitHubAuth private readonly auth: IForgeGitHubAuth,
	...
	async apiRequest<T>(method: string, path: string, body?: unknown): Promise<T> {
		...
		if (!response.ok) {
			...
			this.logService.warn(`[Forge] GitHub API error ${response.status}: ${errorBody}`);
...
registerSingleton(IForgeGitHubService, ForgeGitHubService, InstantiationType.Delayed);
