 ...
 */
export class ForgeSuggestionTooltip extends Disposable {
	...
	constructor(
		...
		this._element.className = `forge-suggestion-tooltip forge-suggestion-severity-${suggestion.severity}`;
	...
	private _render(): void {
		...
		header.className = 'forge-suggestion-tooltip-header';

		const severity = document.createElement('span');
		severity.className = `forge-suggestion-severity-badge`;
		severity.textContent = this.suggestion.severity.toUpperCase();
		header.appendChild(severity);

		const title = document.createElement('span');
		title.className = 'forge-suggestion-tooltip-title';
		...
		desc.className = 'forge-suggestion-tooltip-body';
		...
		if (this.suggestion.suggestedFix) {
			...
			fix.className = 'forge-suggestion-tooltip-fix';
		...
		actions.className = 'forge-suggestion-tooltip-actions';

		if (this.suggestion.suggestedFix) {
			actions.appendChild(this._createButton('Apply Fix', 'forge-btn-primary', () => this.onApply(this.suggestion)));
		}

		actions.appendChild(this._createButton('Explain', 'forge-btn-secondary', () => this.onExplain(this.suggestion)));
		actions.appendChild(this._createButton('Dismiss', 'forge-btn-dismiss', () => this.onDismiss(this.suggestion)));
