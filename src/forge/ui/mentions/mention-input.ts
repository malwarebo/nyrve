...
import { IForgeMentionRegistry, MentionDefinition } from '../../context/mention-registry.js';
 ...
 */
export class ForgeMentionInput extends Disposable {
	...
	constructor(
		parent: HTMLElement,
		private readonly mentionRegistry: IForgeMentionRegistry,
	) {
		super();

		this._dropdown = document.createElement('div');
		this._dropdown.className = 'forge-mention-dropdown';
	...
	private _renderItems(): void {
		...
		for (let i = 0; i < this._matches.length; i++) {
			...
			item.className = 'forge-mention-item';
			if (i === this._selectedIndex) {
				item.classList.add('forge-mention-item-selected');
			}

			const name = document.createElement('span');
			name.className = 'forge-mention-item-name';
			name.textContent = `@${mention.name}`;
			item.appendChild(name);

			const desc = document.createElement('span');
			desc.className = 'forge-mention-item-desc';
			desc.textContent = mention.description;
			item.appendChild(desc);

			if (mention.argumentHint) {
				const hint = document.createElement('span');
				hint.className = 'forge-mention-item-hint';
