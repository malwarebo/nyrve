...
export interface ForgeFileChange {
	...
	readonly originalContent: string;
	readonly proposedContent: string;
	readonly hunks: ForgeHunk[];
}

export interface ForgeChangeSet {
	readonly id: string;
	readonly description: string;
	readonly files: ForgeFileChange[];
...
export const IForgeDiffService = createDecorator<IForgeDiffService>('forgeDiffService');

export interface IForgeDiffService {
	readonly _serviceBrand: undefined;

	/** Fires when a changeset is proposed. */
	readonly onDidProposeChangeSet: Event<ForgeChangeSet>;

	/** Fires when a changeset status changes. */
	readonly onDidChangeStatus: Event<ForgeChangeSet>;

	/** Fires when a hunk status changes. */
	readonly onDidChangeHunkStatus: Event<ForgeHunk>;

	/** Get the current active changeset, if any. */
	getActiveChangeSet(): ForgeChangeSet | undefined;

	/**
	 * Propose a new set of file changes from the agent.
	 * Creates shadow buffers and opens the diff review flow.
	 */
	proposeChanges(description: string, changes: Array<{ filePath: string; proposedContent: string }>): Promise<ForgeChangeSet>;
	...
	getHunks(changeSetId: string): readonly ForgeHunk[];
...
export class ForgeDiffService extends Disposable implements IForgeDiffService {
	declare readonly _serviceBrand: undefined;

	private readonly _onDidProposeChangeSet = this._register(new Emitter<ForgeChangeSet>());
	readonly onDidProposeChangeSet: Event<ForgeChangeSet> = this._onDidProposeChangeSet.event;

	private readonly _onDidChangeStatus = this._register(new Emitter<ForgeChangeSet>());
	readonly onDidChangeStatus: Event<ForgeChangeSet> = this._onDidChangeStatus.event;

	private readonly _onDidChangeHunkStatus = this._register(new Emitter<ForgeHunk>());
	readonly onDidChangeHunkStatus: Event<ForgeHunk> = this._onDidChangeHunkStatus.event;

	private activeChangeSet: ForgeChangeSet | undefined;
	private readonly changeSets = new Map<string, ForgeChangeSet>();
	...
	async proposeChanges(description: string, changes: Array<{ filePath: string; proposedContent: string }>): Promise<ForgeChangeSet> {
		const files: ForgeFileChange[] = [];
		...
		const changeSet: ForgeChangeSet = {
			id: this._generateId(),
			description,
			files,
		...
		this.logService.info(`[Forge] Proposed changeset ${changeSet.id}: ${files.length} files, ${files.reduce((n, f) => n + f.hunks.length, 0)} hunks`);
	...
	async acceptHunk(changeSetId: string, hunkId: string): Promise<void> {
		...
		for (const file of changeSet.files) {
			...
			if (hunk && hunk.status === HunkStatus.Pending) {
				...
				this.logService.trace(`[Forge] Accepted hunk ${hunkId} in ${file.filePath}`);
	...
	rejectHunk(changeSetId: string, hunkId: string): void {
		...
		for (const file of changeSet.files) {
			...
			if (hunk && hunk.status === HunkStatus.Pending) {
				...
				this._onDidChangeHunkStatus.fire(hunk);
				this.logService.trace(`[Forge] Rejected hunk ${hunkId} in ${file.filePath}`);
				break;
			}
		}
	...
	async acceptAll(changeSetId: string): Promise<void> {
		...
		for (const file of changeSet.files) {
			...
			this.undoRedoService.pushElement({
				...
				resource: uri,
				label: `Forge: ${changeSet.description}`,
				code: 'forge.acceptChanges',
				undo: async () => {
		...
		this.logService.info(`[Forge] Accepted all changes in changeset ${changeSetId}`);
	}
	...
	rejectAll(changeSetId: string): void {
		...
		this._onDidChangeStatus.fire(changeSet);
		this.logService.info(`[Forge] Rejected all changes in changeset ${changeSetId}`);
	}
	...
	async openDiffForFile(changeSetId: string, filePath: string): Promise<void> {
		...
		const proposedUri = URI.file(filePath).with({ scheme: 'forge-proposed' });

		await this.editorService.openEditor({
			original: { resource: originalUri },
			modified: { resource: proposedUri },
			label: `Forge: ${filePath}`,
		});
	}
	...
	private async _applyHunk(file: ForgeFileChange, hunk: ForgeHunk): Promise<void> {
		const uri = URI.file(file.filePath);
		const originalContent = file.originalContent;
		...
		this.undoRedoService.pushElement({
			type: UndoRedoElementType.Resource,
			resource: uri,
			label: `Forge: Accept hunk in ${file.filePath}`,
			code: 'forge.acceptHunk',
			undo: async () => {
				await this.textFileService.write(uri, originalContent);
			},
			redo: async () => {
	...
	private _updateChangeSetStatus(changeSet: ForgeChangeSet): void {
	 ...
	 */
	private _computeHunks(filePath: string, original: string, proposed: string): ForgeHunk[] {
		const originalLines = original.split('\n');
		const proposedLines = proposed.split('\n');
		const hunks: ForgeHunk[] = [];
...
registerSingleton(IForgeDiffService, ForgeDiffService, InstantiationType.Delayed);
