...
export class ForgeTaskListRenderer extends Disposable {
	...
	constructor(
		parent: HTMLElement,
		private readonly taskQueue: IForgeTaskQueue,
	) {
		super();
		this._container = document.createElement('div');
		this._container.className = 'forge-task-list';
	...
	private _render(): void {
		...
		if (tasks.length === 0) {
			...
			empty.className = 'forge-task-list-empty';
	...
	private _renderTaskItem(task: Task): HTMLElement {
		...
		row.className = `forge-task-item forge-task-status-${task.status.toLowerCase()}`;

		// Status badge
		const badge = document.createElement('span');
		badge.className = 'forge-task-badge';
		badge.textContent = this._statusLabel(task.status);
		row.appendChild(badge);

		// Title
		const title = document.createElement('span');
		title.className = 'forge-task-title';
		title.textContent = task.title;
		row.appendChild(title);

		// Actions
		const actions = document.createElement('span');
		actions.className = 'forge-task-actions';
	...
	private _actionButton(label: string, onClick: () => void): HTMLButtonElement {
		...
		btn.className = 'forge-task-action-btn';
