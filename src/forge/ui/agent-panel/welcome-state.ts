...
const ttPolicy = createTrustedTypesPolicy('forgeWelcomeState', { createHTML: value => value });
...
export class ForgeWelcomeState extends Disposable {
	...
	render(parent: HTMLElement): HTMLElement {
		this._element = $('div.forge-welcome-state');
		...
		// Flame icon
		const flameContainer = $('div.forge-welcome-flame');
		...
		// Title
		const title = $('div.forge-welcome-title');
		title.textContent = localize('forge.welcome.title', "What are you working on?");
		title.style.cssText = 'font-size: 17px; font-weight: 500; color: #e8e6de; text-align: center; margin-bottom: 8px;';
		this._element.appendChild(title);

		// Subtitle
		const subtitle = $('div.forge-welcome-subtitle');
		subtitle.textContent = localize('forge.welcome.subtitle', "Forge sees your files, terminal, and git. Every change is verified before you see it.");
		...
		const grid = $('div.forge-welcome-grid');
		...
		// Divider
		const divider = $('div.forge-welcome-divider');
	...
	private _createActionCard(action: QuickAction): HTMLElement {
		const card = $('div.forge-action-card');
	...
	private _createProjectStatusSection(): HTMLElement {
		const section = $('div.forge-welcome-status');
		section.style.cssText = 'width: 100%; max-width: 340px;';

		const label = $('div.forge-section-label');
		label.textContent = localize('forge.welcome.projectStatus', "Project status");
	...
	private _createContextMentionsSection(): HTMLElement {
		const section = $('div.forge-welcome-mentions');
		section.style.cssText = 'width: 100%; max-width: 340px; margin-top: 16px;';

		const label = $('div.forge-section-label');
		label.textContent = localize('forge.welcome.contextMentions', "Context mentions");
		label.style.cssText = 'font-size: 11px; font-weight: 500; color: #5F5E5A; letter-spacing: 0.3px; margin-bottom: 8px;';
		section.appendChild(label);

		const row = $('div.forge-chips-row');
		row.style.cssText = 'display: flex; flex-wrap: wrap; gap: 5px;';

		for (const chip of CONTEXT_CHIPS) {
			const chipEl = $('button.forge-chip');
