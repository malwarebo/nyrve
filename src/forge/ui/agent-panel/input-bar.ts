...
const ttPolicy = createTrustedTypesPolicy('forgeInputBar', { createHTML: value => value });
...
export class ForgeInputBar extends Disposable {
	...
	render(parent: HTMLElement): HTMLElement {
		...
		this._statusDot = $('span.forge-status-dot');
		...
		this._statusText.textContent = localize('forge.input.connected', "Connected");
		...
		this._attachmentContainer = $('div.forge-attachment-preview');
		...
		this._textarea.placeholder = localize('forge.input.placeholder', "Ask Forge anything...");
		...
		// Tool buttons
		const toolButtons = $('div.forge-tool-buttons');
		toolButtons.style.cssText = 'display: flex; gap: 1px; align-items: center; padding-right: 2px;';

		toolButtons.appendChild(this._createToolButton(PAPERCLIP_SVG, localize('forge.input.attach', "Attach image"), () => this._onDidClickAttach.fire()));
		toolButtons.appendChild(this._createToolButton(CAMERA_SVG, localize('forge.input.screenshot', "Screenshot"), () => this._onDidClickScreenshot.fire()));
		toolButtons.appendChild(this._createToolButton(AT_SVG, localize('forge.input.mention', "@ mention"), () => this._onDidClickMention.fire()));

		inputWrapper.appendChild(toolButtons);
		inputRow.appendChild(inputWrapper);

		// Send button
		this._sendButton = $('button.forge-send-btn');
	...
	updateStatus(status: InputBarStatus): void {
		if (status.hasApiKey && status.connected) {
			...
			this._statusText.textContent = localize('forge.input.disconnected', "Disconnected");
		} else {
			this._statusDot.style.background = '#EF9F27';
			this._statusText.textContent = localize('forge.input.noApiKey', "No API key");
		}
	...
	showAttachments(attachments: AttachmentInfo[]): void {
		...
		for (const att of attachments.slice(0, 5)) {
			...
			const removeBtn = $('button.forge-att-remove');
			...
			removeBtn.title = localize('forge.input.removeAttachment', "Remove");
	...
	private _createToolButton(iconSvg: string, tooltip: string, onClick: () => void): HTMLElement {
		const btn = $('button.forge-tool-btn');
