...
export interface PanelHeaderOptions {
	...
	readonly availableModels: ForgeModelId[];
...
export class ForgePanelHeader extends Disposable {
	...
	private readonly _onDidChangeModel = this._register(new Emitter<ForgeModelId>());
	readonly onDidChangeModel: Event<ForgeModelId> = this._onDidChangeModel.event;
	...
	private _currentModel: ForgeModelId;
	private _availableModels: ForgeModelId[];
	...
	render(parent: HTMLElement): HTMLElement {
		...
		// Left: mode dropdown + Forge label
		const leftGroup = $('div.forge-header-left');
		...
		const forgeLabel = $('span.forge-label');
		forgeLabel.textContent = 'Forge';
		forgeLabel.style.cssText = 'font-size: 13px; font-weight: 600; color: #d3d1c7;';
		leftGroup.appendChild(forgeLabel);

		this._element.appendChild(leftGroup);

		// Right: model dropdown + icon buttons
		const rightGroup = $('div.forge-header-right');
		...
		// Memory icon
		rightGroup.appendChild(this._createIconButton('\u{1F9E0}', localize('forge.header.memory', "Memory Browser"), () => this._onDidClickMemory.fire()));
		// Settings icon
		rightGroup.appendChild(this._createIconButton('\u2699', localize('forge.header.settings', "Settings"), () => this._onDidClickSettings.fire()));
		// Close icon
		rightGroup.appendChild(this._createIconButton('\u2715', localize('forge.header.close', "Close"), () => this._onDidClickClose.fire()));
	...
	updateModel(model: ForgeModelId): void {
		this._currentModel = model;
		const modelShort = MODEL_DISPLAY[model]?.short ?? model;
		const label = this._modelButton.querySelector('.forge-dropdown-label');
		if (label) {
	...
	private _createDropdownButton(label: string, withIcon: boolean): HTMLElement {
		const btn = $('button.forge-dropdown-btn');
		...
		if (withIcon) {
			const icon = $('span.forge-mode-icon');
			icon.style.cssText = 'width: 10px; height: 10px; border-radius: 50%; border: 1.5px solid #EF9F27;';
			btn.appendChild(icon);
		}

		const lbl = $('span.forge-dropdown-label');
		lbl.textContent = label;
		btn.appendChild(lbl);

		const chevron = $('span.forge-dropdown-chevron');
		chevron.textContent = '\u25BE';
	...
	private _createIconButton(icon: string, tooltip: string, onClick: () => void): HTMLElement {
		const btn = $('button.forge-icon-btn');
	...
	private _toggleModeDropdown(): void {
		...
		this._modeDropdown = $('div.forge-dropdown-menu');
		...
		for (const mode of modes) {
			...
			item.addEventListener('click', () => {
				...
				const modeLabel = this._modeButton.querySelector('.forge-dropdown-label');
				if (modeLabel) {
	...
	private _toggleModelDropdown(): void {
		...
		this._modelDropdown = $('div.forge-dropdown-menu');
