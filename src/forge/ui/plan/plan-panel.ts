...
import { IForgePlanGenerator } from '../../plan/plan-generator.js';
import { IForgePlanExecutor, } from '../../plan/plan-executor.js';
...
export const IForgePlanPanel = createDecorator<IForgePlanPanel>('forgePlanPanel');

export interface IForgePlanPanel {
...
export class ForgePlanPanelService extends Disposable implements IForgePlanPanel {
	...
	constructor(
		@IForgePlanGenerator private readonly planGenerator: IForgePlanGenerator,
		@IForgePlanExecutor private readonly planExecutor: IForgePlanExecutor,
...
<div class="plan-header">
	<span class="plan-title">${localize('forge.plan.title', "Plan Mode")}</span>
</div>
<p style="font-size: 12px; color: var(--vscode-descriptionForeground);">
	${localize('forge.plan.idleMessage', "Describe a complex task and the agent will create a step-by-step plan before writing code.")}
</p>
<textarea class="idle-input" rows="4" placeholder="${localize('forge.plan.placeholder', "e.g., Add authentication with JWT and refresh tokens...")}" id="planInput"></textarea>
<div class="actions-bar">
	<button class="btn btn-primary" onclick="submitPlan()">${localize('forge.plan.generate', "Generate Plan")}</button>
</div>`;
	}
	...
	private _renderGenerating(): string {
		return `
<div class="plan-header">
	<span class="plan-title">${localize('forge.plan.generating', "Generating Plan...")}</span>
</div>
<div class="generating">
	<div class="spinner">&#x23F3;</div>
	<p>${localize('forge.plan.analyzingCodebase', "Analyzing codebase and creating plan...")}</p>
</div>`;
	}
...
${stepsHtml}
<div class="actions-bar">
	<button class="btn btn-secondary" onclick="revisePlan()">${localize('forge.plan.revise', "Revise Plan")}</button>
	<button class="btn btn-secondary" onclick="cancelPlan()">${localize('forge.plan.cancel', "Cancel")}</button>
	<button class="btn btn-primary" onclick="executePlan()">${localize('forge.plan.execute', "Execute Plan")}</button>
</div>`;
	}
...
<div class="plan-header">
	<span class="plan-title">${this._escapeHtml(plan.title)}</span>
	<div>
		<button class="btn btn-secondary" onclick="pauseExecution()">${localize('forge.plan.pause', "Pause")}</button>
		<button class="btn btn-secondary" onclick="cancelPlan()">${localize('forge.plan.stop', "Stop")}</button>
	</div>
</div>
${stepsHtml}`;
	}
	...
	private _renderCompleted(plan: Plan): string {
		...
		return `
<div class="plan-header">
	<span class="plan-title">${localize('forge.plan.completed', "Plan Completed")}</span>
</div>
<div class="plan-meta">
...
<div class="actions-bar">
	<button class="btn btn-secondary" onclick="cancelPlan()">${localize('forge.plan.close', "Close")}</button>
</div>`;
	}
...
registerSingleton(IForgePlanPanel, ForgePlanPanelService, InstantiationType.Delayed);
