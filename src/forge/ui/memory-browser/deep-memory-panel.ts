...
import { IForgeProjectDNA, ProjectDNA } from '../../memory/project-dna.js';
import { IForgeDecisionJournal, DecisionEntry } from '../../memory/decision-journal.js';
import { IForgeTeamKnowledge, TeamKnowledgeSuggestion } from '../../memory/team-knowledge.js';
import { IForgeMemoryRetriever } from '../../memory/memory-retriever.js';
 ...
 */
export class ForgeDeepMemoryBrowser extends Disposable {
	...
	constructor(
		parent: HTMLElement,
		private readonly projectDNA: IForgeProjectDNA,
		private readonly decisionJournal: IForgeDecisionJournal,
		private readonly teamKnowledge: IForgeTeamKnowledge,
		private readonly memoryRetriever: IForgeMemoryRetriever,
	) {
		super();

		this._container = document.createElement('div');
		this._container.className = 'forge-deep-memory-browser';
	...
	private _render(): void {
		...
		content.className = 'forge-deep-memory-content';
	...
	private _renderActiveTab(): void {
		...
		const content = this._container.querySelector('.forge-deep-memory-content');
		if (!content) {
			return;
		}
		content.textContent = '';
	...
	private _renderTabBar(): HTMLElement {
		...
		tabBar.className = 'forge-deep-memory-tabs';
		...
		for (const tab of tabs) {
			...
			btn.className = `forge-deep-memory-tab${this._activeTab === tab.id ? ' active' : ''}`;
	...
	private _renderDNATab(container: HTMLElement): void {
		...
		if (!dna || !dna.lastFullScan) {
			...
			empty.className = 'forge-deep-memory-empty';
			empty.innerHTML = '<p>No DNA scan has been run yet.</p>';

			const scanBtn = document.createElement('button');
			scanBtn.className = 'forge-deep-memory-action-btn';
		...
		rescanBtn.className = 'forge-deep-memory-action-btn';
	...
	private _renderDNASection(title: string, lines: string[]): HTMLElement {
		...
		section.className = 'forge-deep-memory-section';
	...
	private async _renderDecisionsTab(container: HTMLElement): Promise<void> {
		...
		summary.className = 'forge-deep-memory-summary';
	...
	private _renderDecisionEntry(decision: DecisionEntry): HTMLElement {
		...
		row.className = `forge-deep-memory-decision ${decision.status}`;

		// Title + date
		const header = document.createElement('div');
		header.className = 'forge-decision-header';
		const title = document.createElement('strong');
		title.textContent = decision.title;
		header.appendChild(title);
		const date = document.createElement('span');
		date.className = 'forge-decision-date';
		...
		if (decision.rationale) {
			...
			rationale.className = 'forge-decision-rationale';
		...
		meta.className = 'forge-decision-meta';
		const parts = [
		...
		badge.className = `forge-decision-status forge-decision-status-${decision.status}`;
	...
	private async _renderTeamTab(container: HTMLElement): Promise<void> {
		...
		fileInfo.className = 'forge-deep-memory-file-info';
		...
		for (const section of doc.sections) {
			...
			sectionEl.className = 'forge-deep-memory-section';
			...
			for (const entry of section.entries) {
				...
				if (entry.addedDate) {
					...
					dateSpan.className = 'forge-team-entry-date';
	...
	private _renderSuggestionsSection(suggestions: TeamKnowledgeSuggestion[]): HTMLElement {
		const section = document.createElement('div');
		section.className = 'forge-deep-memory-suggestions';
		...
		for (const sug of suggestions) {
			const row = document.createElement('div');
			row.className = 'forge-suggestion-row';

			const content = document.createElement('div');
			content.className = 'forge-suggestion-content';
			content.innerHTML = `<strong>${sug.section}:</strong> ${this._escapeHtml(sug.content)}`;
			row.appendChild(content);

			const reason = document.createElement('div');
			reason.className = 'forge-suggestion-reason';
			reason.textContent = `Reason: ${sug.reason}`;
			row.appendChild(reason);

			const actions = document.createElement('div');
			actions.className = 'forge-suggestion-actions';

			const approveBtn = document.createElement('button');
			approveBtn.className = 'forge-deep-memory-action-btn approve';
			...
			rejectBtn.className = 'forge-deep-memory-action-btn reject';
	...
	private async _renderStatsTab(container: HTMLElement): Promise<void> {
		...
		table.className = 'forge-deep-memory-stats-table';

		for (const stat of stats) {
			const tr = document.createElement('tr');
			const labelTd = document.createElement('td');
			labelTd.className = 'forge-stat-label';
			labelTd.textContent = stat.label;
			tr.appendChild(labelTd);
			const valueTd = document.createElement('td');
			valueTd.className = 'forge-stat-value';
		...
		summarySection.className = 'forge-deep-memory-section';
		...
		summaryPre.className = 'forge-deep-memory-summary-preview';
