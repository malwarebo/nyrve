...
import { IForgeSettingsService } from '../../core/settings-service.js';
import { IForgeAuthService, ConnectionStatus } from '../../core/auth-service.js';
import { IForgeTokenTracker, TokenUsageSummary } from '../../agent/token-tracker.js';
...
/**
 * Controls the Forge Settings Page data flow.
 * Manages state, handles messages from the webview, and pushes updates.
 */
export class ForgeSettingsPageController extends Disposable {
	...
	constructor(
		private readonly settingsService: IForgeSettingsService,
		private readonly authService: IForgeAuthService,
		private readonly tokenTracker: IForgeTokenTracker,
	...
	async handleMessage(msg: SettingsMessage): Promise<void> {
		switch (msg.type) {
			...
			case 'refreshModels':
				...
				break;
			default:
				this.logService.trace(`[Forge] Settings: unknown message type: ${msg.type}`);
		}
	}
	...
	<div class="settings-content">
		...
		<div class="settings-section" id="section-models">
			...
			<div class="setting-item">
				...
				<div class="setting-control">
					<select id="defaultModel" data-key="forge.agent.defaultModel">
			...
			<div class="setting-item">
				...
				<div class="setting-control">
					<select id="complexModel" data-key="forge.agent.complexTaskModel">
			...
			<div class="setting-item">
				...
				<div class="setting-control">
					<select id="bgModel" data-key="forge.agent.backgroundModel">
						<option value="claude-opus">Claude Opus</option>
						<option value="claude-sonnet">Claude Sonnet</option>
			...
			<div class="setting-item">
				...
				<div class="setting-control">
					<label><input type="checkbox" id="modelSwitcher" data-key="forge.agent.modelSwitcher" checked /> Enabled</label>
				</div>
			</div>
		</div>
		...
		<div class="settings-section" id="section-agent">
			...
			<div class="setting-item">
				...
				<div class="setting-control">
					<select id="confirmLevel" data-key="forge.agent.confirmationLevel">
			...
			<div class="setting-item">
				...
				<div class="setting-control">
					<input type="number" id="maxTokens" data-key="forge.agent.maxTokensPerRequest" value="100000" min="1000" max="200000" step="1000" />
				</div>
			</div>
			<div class="setting-item">
				<div class="setting-label">Stream Responses</div>
				<div class="setting-control">
					<label><input type="checkbox" id="streamResp" data-key="forge.agent.streamResponses" checked /> Stream token by token</label>
				</div>
			</div>
		</div>
		...
		<div class="settings-section" id="section-features">
			...
			<div class="setting-item">
				...
				<div class="setting-control">
					<label><input type="checkbox" data-key="forge.backgroundAgent.enabled" checked /> Enabled</label>
				</div>
				<div class="setting-desc" style="margin-top:8px">Mode:</div>
				<div class="setting-control">
					<select data-key="forge.backgroundAgent.mode">
			...
			<div class="setting-item">
				...
				<div class="setting-control">
					<label><input type="checkbox" data-key="forge.indexer.enabled" checked /> Enabled</label>
				</div>
			</div>
			<div class="setting-item">
				<div class="setting-label">Session Memory</div>
				<div class="setting-control">
					<label><input type="checkbox" data-key="forge.memory.enabled" checked /> Enabled</label>
				</div>
			</div>
			<div class="setting-item">
				<div class="setting-label">GitHub Integration</div>
				<div class="setting-control">
					<label><input type="checkbox" data-key="forge.github.enabled" checked /> Enabled</label>
				</div>
			</div>
			<div class="setting-item">
				<div class="setting-label">Diff Review</div>
				<div class="setting-control">
					<label><input type="checkbox" data-key="forge.diff.autoOpenOnChange" checked /> Auto-open on agent changes</label>
				</div>
				<div class="setting-control" style="margin-top:4px">
					<label><input type="checkbox" data-key="forge.diff.showGutterDecorations" checked /> Show gutter decorations</label>
				</div>
			</div>
		</div>
		...
		<div class="settings-section" id="section-privacy">
			<h2>Privacy</h2>
			<p style="font-size:13px;opacity:0.8;margin-bottom:16px;">All Forge data is stored locally on your machine. Your API key is in the OS keychain. Code is sent directly to Anthropic \u2014 there is no Forge backend.</p>
			<div class="setting-item">
				<div class="setting-label">Anonymous Usage Analytics</div>
				<div class="setting-desc">Help improve Forge with anonymous feature usage data. No code or file contents are ever collected.</div>
				<div class="setting-control">
					<label><input type="checkbox" data-key="forge.telemetry.enabled" /> Opt in</label>
				</div>
			</div>
			<div class="setting-item">
				...
				<div class="setting-control">
					<label><input type="checkbox" data-key="forge.memory.cloudSync" /> Opt in</label>
				</div>
			</div>
		</div>
		...
		<div class="settings-section" id="section-advanced">
			...
			<div class="setting-item">
				...
				<div class="setting-control">
					<input type="number" data-key="forge.context.defaultTokenBudget" value="30000" min="1000" max="100000" step="1000" />
				</div>
			</div>
			<div class="setting-item">
				...
				<div class="setting-control">
					<input type="number" data-key="forge.backgroundAgent.dailyTokenBudget" value="500000" min="0" step="10000" />
				</div>
			</div>
			<div class="setting-item">
				...
				<div class="setting-control">
					<input type="number" data-key="forge.tasks.dailyTokenBudget" value="1000000" min="0" step="10000" />
				</div>
			</div>
			<div class="setting-item">
				<div class="setting-label">Memory Max Entries</div>
				<div class="setting-control">
					<input type="number" data-key="forge.memory.maxEntries" value="1000" min="100" step="100" />
				</div>
			</div>
			<div class="setting-item">
				...
				<div class="setting-control">
					<input type="number" data-key="forge.memory.decayDays" value="90" min="7" step="1" />
				</div>
			</div>
		</div>
	</div>
...
const FORGE_SETTINGS_CATEGORY = localize2('forge.settings.category', "Forge");

registerAction2(class OpenForgeSettingsAction extends Action2 {
	constructor() {
		super({
			id: 'forge.openSettings',
			title: localize2('forge.openSettings', "Open Settings"),
			category: FORGE_SETTINGS_CATEGORY,
		});
	}
