...
export const IForgeTypeChecker = createDecorator<IForgeTypeChecker>('forgeTypeChecker');

export interface IForgeTypeChecker {
	readonly _serviceBrand: undefined;

	/** Run the type checker before and after the agent's changes. Returns only NEW errors. */
	check(changeset: ForgeChangeSet): Promise<TypeCheckResult>;
}

// --- Service Implementation ---

export class ForgeTypeChecker extends Disposable implements IForgeTypeChecker {
	...
	async check(changeset: ForgeChangeSet): Promise<TypeCheckResult> {
		const enabled = this.configurationService.getValue<boolean>('forge.verification.runTypeCheck') ?? true;
		if (!enabled) {
		...
		const commandOverride = this.configurationService.getValue<string>('forge.verification.typeCheckCommand');
		const command = commandOverride || config.command;

		this.logService.info(`[Forge] Running type check with ${config.checker}: ${command}`);
		...
		try {
			...
			this.logService.info(
				`[Forge] Type check ${status}: ${newErrors.length} new errors, ` +
			...
			this.logService.error(`[Forge] Type check failed: ${error}`);
			return {
				status: 'fail',
				errorsBefore: [],
				errorsAfter: [],
	...
	private async _runTypeCheckWithStash(
		...
		try {
			if (isBaseline) {
				...
				await this._exec('git stash push -m "forge-verification-baseline"');
			}
	...
	private _skippedResult(reason: string): TypeCheckResult {
		this.logService.info(`[Forge] Type check skipped: ${reason}`);
		return {
			status: 'skipped',
			errorsBefore: [],
			errorsAfter: [],
...
registerSingleton(IForgeTypeChecker, ForgeTypeChecker, InstantiationType.Delayed);
