...
import { IForgeAgentEngine, ForgeMessage } from '../agent-engine.js';
import { IForgeModelRouter } from '../model-router.js';
import { ForgeChangeSet, ForgeFileChange } from '../../ui/diff-review/diff-panel.js';
...
export interface SelfHealResult {
	...
	readonly updatedChangeset: ForgeChangeSet;
}
...
export const IForgeSelfHealer = createDecorator<IForgeSelfHealer>('forgeSelfHealer');

export interface IForgeSelfHealer {
...
export class ForgeSelfHealer extends Disposable implements IForgeSelfHealer {
	...
	async heal(
		...
		const totalTimeout = this.configurationService.getValue<number>('forge.verification.selfHealTimeout') ?? 120000;
		...
		this.logService.info(`[Forge] Self-healing attempt ${attempt}/${maxAttempts} for ${failures.length} failures`);
		...
		try {
			...
			this.logService.info(`[Forge] Self-heal attempt ${attempt} applied ${validFixes.length} fixes in ${elapsed}ms`);
			...
			this.logService.error(`[Forge] Self-heal attempt ${attempt} failed: ${error}`);
			return {
				status: 'could_not_heal',
				fixesApplied: [],
				remainingFailures: failures,
	 ...
	 */
	private _validateFixes(fixes: SelfHealFix[], _changeset: ForgeChangeSet): SelfHealFix[] {
		return fixes.filter(fix => {
			...
			for (const pattern of forbiddenPatterns) {
				if (pattern.test(diff)) {
					this.logService.warn(`[Forge] Self-heal fix rejected: attempted to skip/disable tests in ${fix.filesModified[0]}`);
					return false;
				}
			}
	 ...
	 */
	private _applyFixes(changeset: ForgeChangeSet, fixes: SelfHealFix[]): ForgeChangeSet {
		...
		const updatedFiles: ForgeFileChange[] = changeset.files.map(file => {
...
registerSingleton(IForgeSelfHealer, ForgeSelfHealer, InstantiationType.Delayed);
