...
export const IForgeImportChecker = createDecorator<IForgeImportChecker>('forgeImportChecker');

export interface IForgeImportChecker {
	readonly _serviceBrand: undefined;

	/** Check imports in modified files for broken paths and new circular dependencies. */
	checkImports(changeset: ForgeChangeSet): Promise<ImportCheckResult>;
}

// --- Service Implementation ---

export class ForgeImportChecker extends Disposable implements IForgeImportChecker {
	...
	async checkImports(changeset: ForgeChangeSet): Promise<ImportCheckResult> {
		const enabled = this.configurationService.getValue<boolean>('forge.verification.runImportCheck') ?? true;
		if (!enabled) {
			return { status: 'skipped', brokenImports: [], newCircularDeps: [] };
		}

		this.logService.info('[Forge] Running import check...');
		...
		try {
			...
			this.logService.info(
				`[Forge] Import check ${status}: ${brokenImports.length} broken imports, ` +
				`${newCircularDeps.length} new circular deps`
			);

			return { status, brokenImports, newCircularDeps };
		} catch (error) {
			this.logService.error(`[Forge] Import check failed: ${error}`);
			return { status: 'skipped', brokenImports: [], newCircularDeps: [] };
		}
	}
...
registerSingleton(IForgeImportChecker, ForgeImportChecker, InstantiationType.Delayed);
