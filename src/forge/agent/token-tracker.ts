...
const MODEL_COSTS: Record<ForgeModelId, { input: number; output: number }> = {
...
export interface TokenUsageSummary {
	...
	readonly byModel: Record<ForgeModelId, { inputTokens: number; outputTokens: number; costUsd: number }>;
}

// --- Service Interface ---

export const IForgeTokenTracker = createDecorator<IForgeTokenTracker>('forgeTokenTracker');

export interface IForgeTokenTracker {
	...
	recordUsage(model: ForgeModelId, inputTokens: number, outputTokens: number): TokenUsageRecord;
	...
	calculateCost(model: ForgeModelId, inputTokens: number, outputTokens: number): number;
...
export class ForgeTokenTracker extends Disposable implements IForgeTokenTracker {
	...
	recordUsage(model: ForgeModelId, inputTokens: number, outputTokens: number): TokenUsageRecord {
		...
		this.logService.trace(`[Forge] Token usage: ${model} in=${inputTokens} out=${outputTokens} cost=$${costUsd.toFixed(4)}`);
...
registerSingleton(IForgeTokenTracker, ForgeTokenTracker, InstantiationType.Delayed);
