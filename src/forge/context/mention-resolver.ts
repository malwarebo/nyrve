...
export const IForgeMentionResolver = createDecorator<IForgeMentionResolver>('forgeMentionResolver');

export interface IForgeMentionResolver {
...
export class ForgeMentionResolver extends Disposable implements IForgeMentionResolver {
	...
	async resolve(mention: ParsedMention): Promise<ContextBlock | undefined> {
		try {
			switch (mention.type) {
				...
				default:
					this.logService.trace(`[Forge] Unsupported mention type: ${mention.type}`);
					return undefined;
			}
		} catch (e) {
			this.logService.warn(`[Forge] Failed to resolve @${mention.type}: ${e}`);
			return undefined;
		}
	}
...
registerSingleton(IForgeMentionResolver, ForgeMentionResolver, InstantiationType.Delayed);
