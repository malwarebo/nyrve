...
import { IForgeEditorBridge } from './editor-bridge.js';
import { IForgeMentionResolver } from './mention-resolver.js';
import { IForgeMemoryRetriever, MemoryContext } from '../memory/memory-retriever.js';
...
export const IForgeContextBuilder = createDecorator<IForgeContextBuilder>('forgeContextBuilder');

export interface IForgeContextBuilder {
...
export class ForgeContextBuilder extends Disposable implements IForgeContextBuilder {
	declare readonly _serviceBrand: undefined;

	constructor(
		@IForgeMentionResolver private readonly mentionResolver: IForgeMentionResolver,
		@IForgeEditorBridge private readonly editorBridge: IForgeEditorBridge,
		@IForgeMemoryRetriever private readonly memoryRetriever: IForgeMemoryRetriever,
	...
	async buildContext(userMessage: string): Promise<ContextBuildResult> {
		const tokenBudget = this.configurationService.getValue<number>('forge.context.defaultTokenBudget') ?? 30000;
		...
		this.logService.trace(
			`[Forge] Context built: ${finalBlocks.length} blocks, ${totalTokens} tokens ` +
	...
	private async _retrieveMemoryContext(userMessage: string): Promise<string> {
		try {
			...
			this.logService.trace(
				`[Forge] Memory context: ${memoryResult.totalTokens} tokens ` +
			...
			this.logService.error(`[Forge] Memory retrieval failed: ${error}`);
			return '';
		}
	}
...
registerSingleton(IForgeContextBuilder, ForgeContextBuilder, InstantiationType.Delayed);
