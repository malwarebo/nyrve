...
export const IForgeEncryption = createDecorator<IForgeEncryption>('forgeEncryption');

export interface IForgeEncryption {
 ...
 */
export class ForgeEncryption extends Disposable implements IForgeEncryption {
	...
	async encrypt(plaintext: string): Promise<string> {
		...
		try {
			...
			this.logService.warn(`[Forge] Encryption failed: ${e}`);
	...
	async decrypt(ciphertext: string): Promise<string> {
		...
		try {
			...
			this.logService.warn(`[Forge] Decryption failed: ${e}`);
	...
	async deriveKey(passphrase: string, salt: Uint8Array): Promise<CryptoKey> {
		...
		this.logService.info('[Forge] Encryption key derived successfully');
		return this._key;
	}
}

registerSingleton(IForgeEncryption, ForgeEncryption, InstantiationType.Delayed);
