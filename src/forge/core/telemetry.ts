...
export const IForgeTelemetry = createDecorator<IForgeTelemetry>('forgeTelemetry');

export interface IForgeTelemetry {
...
/**
 * Opt-in anonymous telemetry for Forge usage analytics.
 * Events are buffered locally and only transmitted if the user
 * has explicitly enabled `forge.telemetry.enabled`.
 *
 * No PII, file contents, or code is ever collected.
 * Only high-level feature usage counts and performance metrics.
 */
export class ForgeTelemetry extends Disposable implements IForgeTelemetry {
	...
	logEvent(name: string, properties?: Record<string, string>, measurements?: Record<string, number>): void {
		...
		this.logService.trace(`[Forge] Telemetry event: ${name}`);

		// Prevent unbounded buffer growth
		if (this._buffer.length > ForgeTelemetry.MAX_BUFFER_SIZE) {
			this._buffer.splice(0, this._buffer.length - ForgeTelemetry.MAX_BUFFER_SIZE);
		}
	}
	...
	async flush(): Promise<void> {
		...
		this.logService.trace(`[Forge] Flushing ${events.length} telemetry events`);
...
registerSingleton(IForgeTelemetry, ForgeTelemetry, InstantiationType.Delayed);
