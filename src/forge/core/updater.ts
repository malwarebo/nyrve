...
export const IForgeUpdater = createDecorator<IForgeUpdater>('forgeUpdater');

export interface IForgeUpdater {
...
export class ForgeUpdater extends Disposable implements IForgeUpdater {
	...
	async checkForUpdates(): Promise<UpdateInfo | undefined> {
		...
		try {
			...
			// 1. Fetch https://update.forge.dev/api/latest?platform=<platform>&arch=<arch>
			// 2. Compare version against current
			// 3. Return UpdateInfo if a newer version exists
			this.logService.info('[Forge] Checking for updates...');

			this._setState(UpdateState.Idle);
			return undefined;
		} catch (e) {
			this.logService.warn(`[Forge] Update check failed: ${e}`);
	...
	async downloadUpdate(): Promise<void> {
		...
		try {
			...
			this.logService.info(`[Forge] Downloading update ${this._latestUpdate.version}...`);

			this._setState(UpdateState.Ready);
		} catch (e) {
			this.logService.warn(`[Forge] Update download failed: ${e}`);
			this._setState(UpdateState.Error);
		}
	}
	...
	async applyUpdate(): Promise<void> {
		...
		this.logService.info('[Forge] Applying update and restarting...');
	}
...
registerSingleton(IForgeUpdater, ForgeUpdater, InstantiationType.Delayed);
