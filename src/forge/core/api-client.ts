...
const USER_AGENT = 'Forge-IDE/1.0';
...
export const IForgeApiClient = createDecorator<IForgeApiClient>('forgeApiClient');

export interface IForgeApiClient {
...
export class ForgeApiClient extends Disposable implements IForgeApiClient {
				...
				for (const line of lines) {
					if (line.startsWith('event: ')) {
						...
						try {
							...
							this.logService.trace(`[Forge] SSE parse error: ${data}`);
						}
					}
				}
			}
	...
	async listModels(apiKey: string): Promise<AnthropicModel[]> {
		try {
			...
			if (!response.ok) {
				this.logService.warn(`[Forge] Failed to list models: ${response.status}`);
				return [];
			}
			...
			this.logService.warn(`[Forge] Failed to fetch models: ${e}`);
			return [];
		}
	}
			...
			try {
				...
				if (response.status === 429 || response.status === 529) {
					if (attempt < MAX_RETRIES) {
						...
						this.logService.trace(`[Forge] API rate limited (${response.status}), retrying in ${delayMs}ms (attempt ${attempt + 1}/${MAX_RETRIES})`);
				...
				if (attempt < MAX_RETRIES) {
					...
					this.logService.trace(`[Forge] API request failed, retrying in ${delayMs}ms: ${lastError.message}`);
					await this._sleep(delayMs);
				}
			}
		}
...
registerSingleton(IForgeApiClient, ForgeApiClient, InstantiationType.Delayed);
