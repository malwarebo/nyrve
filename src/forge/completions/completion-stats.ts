...
import { IForgeCompletionCache } from './completion-cache.js';
...
export const IForgeCompletionStats = createDecorator<IForgeCompletionStats>('forgeCompletionStats');

export interface IForgeCompletionStats {
...
export class ForgeCompletionStats extends Disposable implements IForgeCompletionStats {
	...
	record(event: CompletionEvent): void {
		...
		this.logService.trace(`[Forge] Completion stat: ${event.type} (${event.language}, ${event.latencyMs}ms, cached=${event.cached})`);
	}
...
registerSingleton(IForgeCompletionStats, ForgeCompletionStats, InstantiationType.Delayed);
