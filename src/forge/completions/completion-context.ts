...
import { IForgeEditorBridge } from '../context/editor-bridge.js';
import { IForgeProjectDNA } from '../memory/project-dna.js';
...
export const IForgeCompletionContext = createDecorator<IForgeCompletionContext>('forgeCompletionContext');

export interface IForgeCompletionContext {
...
export class ForgeCompletionContextService extends Disposable implements IForgeCompletionContext {
	...
	buildRequest(triggerKind: TriggerKind): CompletionRequest | undefined {
		...
		// Project conventions
		const useProjectContext = this.configurationService.getValue<boolean>('forge.completions.useProjectContext') ?? true;
	...
	buildPrompt(request: CompletionRequest): CompletionPrompt {
		...
		this.logService.trace(`[Forge] Completion context: ~${estimatedTokens} tokens`);
...
registerSingleton(IForgeCompletionContext, ForgeCompletionContextService, InstantiationType.Delayed);
