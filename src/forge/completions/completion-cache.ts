...
export const IForgeCompletionCache = createDecorator<IForgeCompletionCache>('forgeCompletionCache');

export interface IForgeCompletionCache {
...
export class ForgeCompletionCache extends Disposable implements IForgeCompletionCache {
	...
	lookup(prefix: string, suffix: string, lineText: string): CacheLookupResult {
		...
		// Check TTL
		const ttl = (this.configurationService.getValue<number>('forge.completions.cacheTTL') ?? 30) * 1000;
	...
	store(prefix: string, suffix: string, lineText: string, completion: CachedCompletion): void {
		const maxSize = this.configurationService.getValue<number>('forge.completions.cacheSize') ?? 100;
...
registerSingleton(IForgeCompletionCache, ForgeCompletionCache, InstantiationType.Delayed);
