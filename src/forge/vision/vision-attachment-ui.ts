...
import { IForgeImageInput, ImageAttachment } from './image-input.js';
...
export const IForgeVisionAttachmentUI = createDecorator<IForgeVisionAttachmentUI>('forgeVisionAttachmentUI');

export interface IForgeVisionAttachmentUI {
...
export class ForgeVisionAttachmentUI extends Disposable implements IForgeVisionAttachmentUI {
	...
	constructor(
		@IForgeImageInput private readonly imageInput: IForgeImageInput,
...
<div class="attachment-preview" data-id="${a.id}" style="display: inline-block; position: relative; margin: 4px;">
	...
	<button class="attachment-remove" data-id="${a.id}" style="position: absolute; top: -4px; right: -4px; width: 16px; height: 16px; border-radius: 50%; background: var(--vscode-errorForeground); color: white; border: none; cursor: pointer; font-size: 10px; line-height: 16px; padding: 0;" title="${localize('forge.vision.remove', "Remove image")}">\u00d7</button>
</div>`).join('');
...
<div class="chat-image" style="margin: 8px 0;">
	<img src="data:${attachment.mimeType};base64,${attachment.thumbnailBase64}" alt="${this._escapeHtml(attachment.originalFilename ?? 'image')}" style="max-width: 300px; max-height: 200px; border-radius: 4px; border: 1px solid var(--vscode-panel-border); cursor: pointer;" title="${localize('forge.vision.clickToEnlarge', "Click to enlarge")}" />
...
<div class="vision-toolbar" style="display: inline-flex; gap: 4px;">
	<button class="vision-btn" data-action="file-picker" title="${localize('forge.vision.attachImage', "Attach Image")}" style="background: none; border: none; cursor: pointer; padding: 4px; color: var(--vscode-foreground);">\u{1F4CE}</button>
	<button class="vision-btn" data-action="screenshot" title="${localize('forge.vision.screenshot', "Capture Screenshot")}" style="background: none; border: none; cursor: pointer; padding: 4px; color: var(--vscode-foreground);">\u{1F4F7}</button>
</div>`;
	}
...
registerSingleton(IForgeVisionAttachmentUI, ForgeVisionAttachmentUI, InstantiationType.Delayed);
