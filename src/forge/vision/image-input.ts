...
export const IForgeImageInput = createDecorator<IForgeImageInput>('forgeImageInput');

export interface IForgeImageInput {
...
export class ForgeImageInput extends Disposable implements IForgeImageInput {
	...
	async captureScreenshot(): Promise<ImageAttachment | undefined> {
		...
		this.logService.info('[Forge] Screenshot capture requested (platform-specific)');
		return undefined;
	}
	...
	private async _processFile(file: File, source: ImageSource): Promise<ImageAttachment | undefined> {
		try {
			...
			this.logService.trace(`[Forge] Image attached: ${source} (${file.name}, ${file.size} bytes)`);
			return attachment;
		} catch (e) {
			this.logService.warn('[Forge] Failed to process image file', e);
			return undefined;
		}
	}
	...
	private async _processUri(uri: URI, source: ImageSource): Promise<ImageAttachment | undefined> {
		try {
			...
			if (!mimeType) {
				this.logService.warn(`[Forge] Unsupported image format: ${ext}`);
				return undefined;
			}
			...
			this._attachments.push(attachment);
			this._onDidAttachImage.fire(attachment);
			this.logService.trace(`[Forge] Image attached from URI: ${source}`);
			return attachment;
		} catch (e) {
			this.logService.warn('[Forge] Failed to process image URI', e);
			return undefined;
		}
	}
...
registerSingleton(IForgeImageInput, ForgeImageInput, InstantiationType.Delayed);
