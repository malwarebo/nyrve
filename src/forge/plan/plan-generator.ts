...
export const IForgePlanGenerator = createDecorator<IForgePlanGenerator>('forgePlanGenerator');

export interface IForgePlanGenerator {
...
export class ForgePlanGenerator extends Disposable implements IForgePlanGenerator {
	...
	async generatePlan(request: PlanRequest): Promise<Plan> {
		...
		const modelId = this.modelRouter.selectModel(ForgeTaskComplexity.High);
		...
		this.logService.info(`[Forge] Plan generated: "${plan.title}" with ${plan.steps.length} steps`);
		return plan;
	}
...
Revise the plan based on the feedback. Output the complete revised plan as JSON.`;
		...
		this.logService.info(`[Forge] Plan revised: "${updatedPlan.title}"`);
		return updatedPlan;
	}
	...
	private _parsePlanResponse(response: string, request: PlanRequest, memoryUsed: string[]): Plan {
		...
		try {
			...
			this.logService.warn('[Forge] Failed to parse plan response as JSON, creating fallback');
			parsed = {
...
registerSingleton(IForgePlanGenerator, ForgePlanGenerator, InstantiationType.Delayed);
