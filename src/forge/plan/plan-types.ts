...
export const IForgePlanStorage = createDecorator<IForgePlanStorage>('forgePlanStorage');

export interface IForgePlanStorage {
...
export class ForgePlanStorage extends Disposable implements IForgePlanStorage {
	...
	async save(plan: Plan): Promise<void> {
		...
		try {
			...
			this.logService.trace(`[Forge] Plan saved: ${plan.id}`);
		} catch (e) {
			this.logService.error(`[Forge] Failed to save plan: ${plan.id}`, e);
		}
	}
	...
	private _planUri(planId: string): URI {
		...
		if (!workspaceFolder) {
			return URI.file(`.forge/plans/${planId}.json`);
		}
		return URI.joinPath(workspaceFolder.uri, '.forge', 'plans', `${planId}.json`);
	}

	private _plansDir(): URI {
		const workspaceFolder = this.workspaceContextService.getWorkspace().folders[0];
		if (!workspaceFolder) {
			return URI.file('.forge/plans');
		}
		return URI.joinPath(workspaceFolder.uri, '.forge', 'plans');
	}
}

registerSingleton(IForgePlanStorage, ForgePlanStorage, InstantiationType.Delayed);
