...
import { NyrveSymbol, INyrveSymbolExtractor } from './symbol-extractor.js';
...
export const enum NyrveIndexState {
	Idle = 'idle',
	Indexing = 'indexing',
	Ready = 'ready',
	Error = 'error',
...
export const INyrveIndexManager = createDecorator<INyrveIndexManager>('nyrveIndexManager');

export interface INyrveIndexManager {
	readonly _serviceBrand: undefined;

	/** Fires when indexing state changes. */
	readonly onDidChangeState: Event<NyrveIndexState>;
	...
	readonly state: NyrveIndexState;
	...
	getFileSymbols(filePath: string): readonly NyrveSymbol[];
...
export class NyrveIndexManager extends Disposable implements INyrveIndexManager {
	declare readonly _serviceBrand: undefined;

	private readonly _onDidChangeState = this._register(new Emitter<NyrveIndexState>());
	readonly onDidChangeState: Event<NyrveIndexState> = this._onDidChangeState.event;
	...
	private _state: NyrveIndexState = NyrveIndexState.Idle;
	private readonly _index = new Map<string, FileIndexEntry>();
	private readonly _symbolIndex = new Map<string, NyrveSymbol[]>(); // symbol name → occurrences
	private readonly _watcherDisposables = this._register(new DisposableStore());

	get state(): NyrveIndexState {
		return this._state;
	}
	...
	constructor(
		...
		@INyrveIgnoreService private readonly ignoreService: INyrveIgnoreService,
		@INyrveSymbolExtractor private readonly symbolExtractor: INyrveSymbolExtractor,
	...
	async buildIndex(): Promise<void> {
		const enabled = this.configurationService.getValue<boolean>('nyrve.indexer.enabled') ?? true;
		if (!enabled) {
			return;
		}

		this._setState(NyrveIndexState.Indexing);
		await this.ignoreService.reload();

		const workspace = this.workspaceService.getWorkspace();
		const projectRoot = workspace.folders[0]?.uri;
		if (!projectRoot) {
			this._setState(NyrveIndexState.Ready);
			return;
		}

		try {
			const maxFiles = this.configurationService.getValue<number>('nyrve.indexer.maxProjectFiles') ?? 50000;
			const files = await this._collectFiles(projectRoot, maxFiles);
			const total = files.length;

			this.logService.info(`[Nyrve] Starting index build: ${total} files`);
			...
			for (const fileUri of files) {
				...
				try {
					...
					this.logService.warn(`[Nyrve] Failed to index ${filePath}: ${e}`);
				}
			...
			this._setState(NyrveIndexState.Ready);
			this.logService.info(`[Nyrve] Index build complete: ${this._index.size} files, ${this._countTotalSymbols()} symbols`);
		} catch (e) {
			this.logService.error(`[Nyrve] Index build failed: ${e}`);
			this._setState(NyrveIndexState.Error);
		}
	}
	...
	async updateFile(filePath: string): Promise<void> {
		...
		try {
			...
			this.logService.trace(`[Nyrve] Updated index for ${filePath}`);
		} catch (e) {
			this.logService.warn(`[Nyrve] Failed to update index for ${filePath}: ${e}`);
		}
	}
	...
	getStats(): IndexStats {
		return {
			...
			isIndexing: this._state === NyrveIndexState.Indexing,
	...
	private _setState(state: NyrveIndexState): void {
...
registerSingleton(INyrveIndexManager, NyrveIndexManager, InstantiationType.Delayed);
