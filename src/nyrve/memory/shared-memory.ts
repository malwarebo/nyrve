...
export const INyrveSharedMemory = createDecorator<INyrveSharedMemory>('nyrveSharedMemory');

export interface INyrveSharedMemory {
	readonly _serviceBrand: undefined;

	/** Load shared memory from .nyrve/shared-memory.json. */
	load(): Promise<void>;
...
export class NyrveSharedMemory extends Disposable implements INyrveSharedMemory {
	...
	async load(): Promise<void> {
		...
		const uri = URI.joinPath(projectRoot, '.nyrve', 'shared-memory.json');
		try {
			...
			this.logService.info('[Nyrve] Loaded shared memory');
		} catch (e) {
			this.logService.warn(`[Nyrve] Failed to load shared memory: ${e}`);
			this._data = undefined;
		}
	}
...
registerSingleton(INyrveSharedMemory, NyrveSharedMemory, InstantiationType.Delayed);
