...
/**
 * Nyrve Native AI Removal
 *
 * This module disables VS Code's built-in Chat/Copilot/Inline Chat features
 * so that only the Nyrve Agent Panel is visible. Rather than deleting upstream
 ...
 *
 * Import order in workbench entry points (changes marked with "// [Nyrve] Disabled"):
 ...
 */
export class NyrveDisableNativeAI extends Disposable {
	static readonly ID = 'nyrve.disableNativeAI';
	...
	constructor(
		...
		this.logService.info('[Nyrve] Native Chat/Copilot features disabled');
	}
		...
		try {
			...
			if (chatContainer) {
				...
				if (views.length > 0) {
					...
					this.logService.trace(`[Nyrve] Removed ${views.length} chat views — container will auto-hide`);
				}
			}
		} catch (e) {
			this.logService.warn('[Nyrve] Failed to hide chat view container', e);
		}
	}
	...
	private _deregisterChatCommands(): void {
		...
		if (removedCount > 0) {
			this.logService.trace(`[Nyrve] Replaced ${removedCount} chat/copilot commands with no-ops`);
		}
	}
...
registerWorkbenchContribution2(
	NyrveDisableNativeAI.ID,
	NyrveDisableNativeAI,
