...
const ANTHROPIC_API_KEY_SECRET = 'nyrve.anthropic.apiKey';
...
export const INyrveAuthService = createDecorator<INyrveAuthService>('nyrveAuthService');

export interface INyrveAuthService {
...
export class NyrveAuthService extends Disposable implements INyrveAuthService {
	...
	async deleteApiKey(): Promise<void> {
		...
		this.logService.info('[Nyrve] Anthropic API key removed');
	}
	...
	async getAvailableModels(): Promise<readonly ClaudeModel[]> {
		...
		this.logService.info(`[Nyrve] Discovered ${this._cachedModels.length} Claude models`);
	...
	private _startHealthCheck(): void {
		...
		this._healthCheckTimer = setInterval(() => {
			this.checkConnection().catch(e => {
				this.logService.trace(`[Nyrve] Health check failed: ${e}`);
			});
...
registerSingleton(INyrveAuthService, NyrveAuthService, InstantiationType.Delayed);
