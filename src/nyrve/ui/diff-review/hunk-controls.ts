...
import { NyrveHunk, HunkStatus, INyrveDiffService } from './diff-panel.js';

/**
 * Editor contribution that renders per-hunk accept/reject controls
 * as view zones in the diff editor gutter area.
 */
export class NyrveHunkControlsContribution extends Disposable implements IEditorContribution {

	static readonly ID = 'nyrve.hunkControlsContribution';
	...
	private _addHunkZone(accessor: IViewZoneChangeAccessor, changeSetId: string, hunk: NyrveHunk): void {
	...
	private _createHunkControlsDom(changeSetId: string, hunk: NyrveHunk): HTMLElement {
		const container = $('div.nyrve-hunk-controls');
		...
		// Accept button
		const acceptBtn = $('button.nyrve-hunk-accept');
		acceptBtn.textContent = localize('nyrve.hunk.accept', "Accept");
		acceptBtn.title = localize('nyrve.hunk.acceptTitle', "Accept this change (Cmd+Shift+Y)");
		...
		// Reject button
		const rejectBtn = $('button.nyrve-hunk-reject');
		rejectBtn.textContent = localize('nyrve.hunk.reject', "Reject");
		rejectBtn.title = localize('nyrve.hunk.rejectTitle', "Reject this change (Cmd+Shift+N)");
		...
		// Line range label
		const label = $('span.nyrve-hunk-label');
		label.style.cssText = 'opacity: 0.7; margin-left: 8px;';
		label.textContent = hunk.startLine === hunk.endLine
			? localize('nyrve.hunk.lineLabel', "Line {0}", hunk.startLine)
			: localize('nyrve.hunk.linesLabel', "Lines {0}-{1}", hunk.startLine, hunk.endLine);
...
registerEditorContribution(NyrveHunkControlsContribution.ID, NyrveHunkControlsContribution, 0 /* EditorContributionInstantiation.Eager */);
