...
import { INyrveMemoryEngine, MemoryEntry, MemoryType, MemorySource } from '../../memory/memory-engine.js';

/**
 * Memory browser panel — displays all memories with search, filter,
 * edit, verify, and delete capabilities.
 */
export class NyrveMemoryBrowser extends Disposable {
	...
	constructor(
		parent: HTMLElement,
		private readonly memoryEngine: INyrveMemoryEngine,
	) {
		super();

		this._container = document.createElement('div');
		this._container.className = 'nyrve-memory-browser';
	...
	private _render(): void {
		...
		if (memories.length === 0) {
			...
			empty.className = 'nyrve-memory-empty';
		...
		list.className = 'nyrve-memory-list';
	...
	private _renderToolbar(): HTMLElement {
		...
		toolbar.className = 'nyrve-memory-toolbar';

		// Search input
		const searchInput = document.createElement('input');
		searchInput.className = 'nyrve-memory-search';
		...
		filter.className = 'nyrve-memory-filter';
	...
	private _renderStats(memories: readonly MemoryEntry[]): HTMLElement {
		...
		stats.className = 'nyrve-memory-stats';
	...
	private _renderEntry(entry: MemoryEntry): HTMLElement {
		...
		actions.className = 'nyrve-memory-actions';
	...
	private _actionBtn(label: string, onClick: () => void): HTMLButtonElement {
		...
		btn.className = 'nyrve-memory-action-btn';
