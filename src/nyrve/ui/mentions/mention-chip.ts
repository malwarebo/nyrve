 ...
 */
export class NyrveMentionChip extends Disposable {

	private readonly _onDidRemove = this._register(new Emitter<NyrveMentionChip>());
	...
	constructor(
		...
		this._element.className = `nyrve-mention-chip nyrve-mention-type-${mention.type}`;

		const label = document.createElement('span');
		label.className = 'nyrve-mention-chip-label';
		label.textContent = argument ? `@${mention.name} ${argument}` : `@${mention.name}`;
		this._element.appendChild(label);

		const removeBtn = document.createElement('span');
		removeBtn.className = 'nyrve-mention-chip-remove';
 ...
 */
export class NyrveMentionChipContainer extends Disposable {

	private readonly _chips: NyrveMentionChip[] = [];
	...
	constructor(parent: HTMLElement) {
		...
		this._container.className = 'nyrve-mention-chips';
		parent.appendChild(this._container);
	}

	addChip(mention: MentionDefinition, argument?: string): NyrveMentionChip {
		const chip = new NyrveMentionChip(this._container, mention, argument);
	...
	getChips(): readonly NyrveMentionChip[] {
		return this._chips;
	}
