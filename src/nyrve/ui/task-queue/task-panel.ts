...
export const INyrveTaskQueue = createDecorator<INyrveTaskQueue>('nyrveTaskQueue');

export interface INyrveTaskQueue {
...
export class NyrveTaskQueue extends Disposable implements INyrveTaskQueue {
	...
	addTask(params: TaskCreateParams): Task {
		...
		this.logService.info(`[Nyrve] Task queued: ${task.title} (${task.id})`);
	...
	cancelTask(id: string): void {
		...
		this.logService.info(`[Nyrve] Task cancelled: ${task.title}`);
	}
	...
	pauseTask(id: string): void {
		...
		this._onDidUpdateTask.fire(task);
		this.logService.info(`[Nyrve] Task paused: ${task.title}`);
	}
	...
	resumeTask(id: string): void {
		...
		this._onDidUpdateTask.fire(task);
		this.logService.info(`[Nyrve] Task resumed: ${task.title}`);
	}
	...
	processNext(): void {
		const maxConcurrent = this.configurationService.getValue<number>('nyrve.tasks.maxConcurrent') ?? 1;
		...
		this.logService.info(`[Nyrve] Task started: ${next.title}`);
	}
	...
	completeTask(id: string): void {
		...
		task.completedAt = new Date().toISOString();
		this._onDidUpdateTask.fire(task);
		this.logService.info(`[Nyrve] Task completed: ${task.title}`);
	}
	...
	failTask(id: string, error: string): void {
		...
		task.error = error;
		this._onDidUpdateTask.fire(task);
		this.logService.warn(`[Nyrve] Task failed: ${task.title} — ${error}`);
	}
...
registerSingleton(INyrveTaskQueue, NyrveTaskQueue, InstantiationType.Delayed);
