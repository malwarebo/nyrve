...
const ttPolicy = createTrustedTypesPolicy('nyrveMessageRenderer', { createHTML: value => value });
...
export class NyrveMessageRenderer extends Disposable {
	...
	render(parent: HTMLElement): HTMLElement {
		this._container = $('div.nyrve-messages');
	...
	appendUserMessage(content: string, imageHtml?: string): void {
		...
		if (imageHtml) {
			const imgContainer = $('div.nyrve-msg-image');
	...
	appendAssistantMessage(content: string, verification?: VerificationSummary): void {
		this._removeThinking();
		const msg = $('div.nyrve-msg.nyrve-msg-assistant');
		msg.style.cssText = 'padding: 12px 0; border-bottom: 1px solid #2e2d28;';
		...
		const label = $('span.nyrve-msg-label');
		label.textContent = localize('nyrve.msg.nyrve', "Nyrve");
	...
	showThinking(actionText?: string): void {
		...
		this._thinkingElement = $('div.nyrve-msg.nyrve-msg-thinking');
		...
		// Animated dots
		const dotsRow = $('div.nyrve-thinking-dots');
		dotsRow.style.cssText = 'display: flex; gap: 4px; margin-bottom: 6px;';

		for (let i = 0; i < 3; i++) {
			const dot = $('span.nyrve-dot');
			dot.style.cssText = `width: 6px; height: 6px; border-radius: 50%; background: #EF9F27; animation: nyrvePulse 0.6s ease-in-out infinite; animation-delay: ${i * 0.15}s;`;
		...
		// Action text
		const action = $('div.nyrve-thinking-action');
	...
	updateThinkingAction(actionText: string): void {
		if (this._thinkingElement) {
			const action = this._thinkingElement.querySelector('.nyrve-thinking-action');
			if (action) {
				action.textContent = actionText;
			}
		}
	...
	startStreaming(): void {
		this._removeThinking();
		const msg = $('div.nyrve-msg.nyrve-msg-assistant.nyrve-msg-streaming');
		msg.style.cssText = 'padding: 12px 0; border-bottom: 1px solid #2e2d28;';
	...
	updateStreaming(content: string): void {
		const el = this._container.querySelector('.nyrve-msg-streaming .nyrve-msg-text') as HTMLElement | null;
		if (el) {
	...
	finalizeStreaming(): void {
		const el = this._container.querySelector('.nyrve-msg-streaming');
		if (el) {
			el.classList.remove('nyrve-msg-streaming');
		}
	}

	appendErrorMessage(error: string): void {
		this._removeThinking();
		const msg = $('div.nyrve-msg.nyrve-msg-error');
		msg.style.cssText = 'padding: 12px 0; border-bottom: 1px solid #2e2d28;';
	...
	private _createVerificationSummary(v: VerificationSummary): HTMLElement {
		const box = $('div.nyrve-verification');
	...
	private _ensurePulseAnimation(): void {
		if (document.getElementById('nyrve-pulse-style')) {
			return;
		}
		const style = document.createElement('style');
		style.id = 'nyrve-pulse-style';
		style.textContent = '@keyframes nyrvePulse { 0%, 100% { opacity: 0.3; } 50% { opacity: 1; } }';
