...
export const INyrveImageProcessor = createDecorator<INyrveImageProcessor>('nyrveImageProcessor');

export interface INyrveImageProcessor {
...
export class NyrveImageProcessor extends Disposable implements INyrveImageProcessor {
	...
	async process(attachment: ImageAttachment): Promise<ProcessedImage> {
		const maxDim = this.configurationService.getValue<number>('nyrve.vision.maxImageDimension') ?? MAX_DIMENSION;
		...
		if (width > maxDim || height > maxDim) {
			...
			wasResized = true;
			this.logService.trace(`[Nyrve] Image resized to ${width}x${height}`);
		}
		...
		if (width < MIN_DIMENSION || height < MIN_DIMENSION) {
			...
			height = Math.round(height * scale);
			wasResized = true;
			this.logService.trace(`[Nyrve] Small image upscaled to ${width}x${height}`);
		}

		// Convert GIF to PNG (first frame only)
		if (mimeType === 'image/gif') {
			mimeType = 'image/png';
			this.logService.trace('[Nyrve] GIF converted to PNG');
		}
		...
		if (sizeBytes > MAX_SIZE_BYTES) {
			...
			this.logService.trace(`[Nyrve] Image compressed (${sizeBytes} bytes)`);
		}
...
registerSingleton(INyrveImageProcessor, NyrveImageProcessor, InstantiationType.Delayed);
