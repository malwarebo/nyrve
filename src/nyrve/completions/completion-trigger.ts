...
import { INyrveAgentService, NyrveAgentState } from '../agent/agent-service.js';
...
export const INyrveCompletionTrigger = createDecorator<INyrveCompletionTrigger>('nyrveCompletionTrigger');

export interface INyrveCompletionTrigger {
...
export class NyrveCompletionTrigger extends Disposable implements INyrveCompletionTrigger {
	...
	shouldTrigger(event: EditorChangeEvent): TriggerDecision {
		...
		if (agentState === NyrveAgentState.Thinking || agentState === NyrveAgentState.Streaming) {
		...
		const baseDelay = this.configurationService.getValue<number>('nyrve.completions.triggerDelay') ?? 150;
	...
	recordRateLimit(): void {
		...
		this.logService.info('[Nyrve] Completions: rate limit hit, backing off');
	}
	...
	private _isLanguageEnabled(languageId: string): boolean {
		const enabledLanguages = this.configurationService.getValue<string[]>('nyrve.completions.enabledLanguages') ?? ['*'];
		const disabledLanguages = this.configurationService.getValue<string[]>('nyrve.completions.disabledLanguages') ?? [];
...
registerSingleton(INyrveCompletionTrigger, NyrveCompletionTrigger, InstantiationType.Delayed);
