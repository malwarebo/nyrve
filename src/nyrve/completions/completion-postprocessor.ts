...
export const INyrveCompletionPostProcessor = createDecorator<INyrveCompletionPostProcessor>('nyrveCompletionPostProcessor');

export interface INyrveCompletionPostProcessor {
...
export class NyrveCompletionPostProcessor extends Disposable implements INyrveCompletionPostProcessor {
	...
	private _enforceLimit(text: string, modifications: string[]): string {
		const maxLines = this.configurationService.getValue<number>('nyrve.completions.maxLines') ?? MAX_COMPLETION_LINES;
		const lines = text.split('\n');
...
registerSingleton(INyrveCompletionPostProcessor, NyrveCompletionPostProcessor, InstantiationType.Delayed);
