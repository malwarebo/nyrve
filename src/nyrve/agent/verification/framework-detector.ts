...
export const INyrveFrameworkDetector = createDecorator<INyrveFrameworkDetector>('nyrveFrameworkDetector');

export interface INyrveFrameworkDetector {
...
export class NyrveFrameworkDetector extends Disposable implements INyrveFrameworkDetector {
	...
	async detect(): Promise<FrameworkDetectionResult> {
		...
		if (!root) {
			this.logService.warn('[Nyrve] No workspace root found for framework detection');
			return { testRunner: NO_TEST_RUNNER, typeChecker: NO_TYPE_CHECKER };
		}

		this.logService.info('[Nyrve] Detecting project frameworks...');
		...
		this.logService.info(
			`[Nyrve] Detection complete: test=${testRunner.detected ? testRunner.framework : 'none'}, ` +
...
registerSingleton(INyrveFrameworkDetector, NyrveFrameworkDetector, InstantiationType.Delayed);
