...
export const INyrveTestRunner = createDecorator<INyrveTestRunner>('nyrveTestRunner');

export interface INyrveTestRunner {
...
export class NyrveTestRunner extends Disposable implements INyrveTestRunner {
	...
	async runTests(changeset: NyrveChangeSet): Promise<TestResult> {
		const enabled = this.configurationService.getValue<boolean>('nyrve.verification.runTests') ?? true;
		if (!enabled) {
		...
		const testTimeout = this.configurationService.getValue<number>('nyrve.verification.testTimeout') ?? 60000;

		// Allow user to override the auto-detected command
		const commandOverride = this.configurationService.getValue<string>('nyrve.verification.testCommand');
		...
		try {
			...
			this.logService.info(
				`[Nyrve] Tests ${status}: ${afterRun.passed} passed, ${afterRun.failed} failed, ` +
			...
			this.logService.error(`[Nyrve] Test run failed: ${error}`);
			return {
				status: 'fail',
				framework: config.framework,
	...
	private _skippedResult(reason: string): TestResult {
		this.logService.info(`[Nyrve] Tests skipped: ${reason}`);
		return {
			status: 'skipped',
			framework: '',
			command: '',
...
registerSingleton(INyrveTestRunner, NyrveTestRunner, InstantiationType.Delayed);
