...
import { INyrveTypeChecker, TypeCheckResult } from './verification/type-checker.js';
import { INyrveTestRunner, TestResult } from './verification/test-runner.js';
import { INyrveCoverageChecker, CoverageResult } from './verification/coverage-checker.js';
import { INyrveImportChecker, ImportCheckResult } from './verification/import-checker.js';
import { INyrveSelfHealer, VerificationAttempt } from './verification/self-healer.js';
import { INyrveFrameworkDetector, TestRunnerConfig, TypeCheckerConfig } from './verification/framework-detector.js';
import {
	VerificationReport,
...
export const INyrveVerificationEngine = createDecorator<INyrveVerificationEngine>('nyrveVerificationEngine');
...
export class NyrveVerificationEngine extends Disposable implements INyrveVerificationEngine {
	...
	constructor(
		...
		@INyrveCoverageChecker private readonly coverageChecker: INyrveCoverageChecker,
	...
	async verify(changeset: NyrveChangeSet): Promise<VerificationReport> {
		const enabled = this.configurationService.getValue<boolean>('nyrve.verification.enabled') ?? true;
		if (!enabled) {
		...
		while (attempt <= maxAttempts) {
			this.logService.info(`[Nyrve] Verification pipeline${attempt > 0 ? ` (after heal attempt ${attempt})` : ''}`);
...
registerSingleton(INyrveVerificationEngine, NyrveVerificationEngine, InstantiationType.Delayed);
