...
export const INyrveMentionRegistry = createDecorator<INyrveMentionRegistry>('nyrveMentionRegistry');

export interface INyrveMentionRegistry {
	...
	/** Get custom mentions from .nyrve/mentions.json. */
	...
	/** Reload custom mentions from .nyrve/mentions.json. */
...
export class NyrveMentionRegistry extends Disposable implements INyrveMentionRegistry {
	...
	async reloadCustomMentions(): Promise<void> {
		...
		const mentionsUri = URI.joinPath(projectRoot, '.nyrve', 'mentions.json');
		try {
			...
			this.logService.info(`[Nyrve] Loaded ${this._customMentions.length} custom mentions`);
		} catch (e) {
			this.logService.warn(`[Nyrve] Failed to load custom mentions: ${e}`);
			this._customMentions = [];
		}
	}
}

registerSingleton(INyrveMentionRegistry, NyrveMentionRegistry, InstantiationType.Delayed);
