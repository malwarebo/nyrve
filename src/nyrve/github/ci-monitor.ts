...
import { GitHubWorkflowRun, INyrveGitHubService } from './github-service.js';
...
export const INyrveCIMonitor = createDecorator<INyrveCIMonitor>('nyrveCIMonitor');

export interface INyrveCIMonitor {
...
export class NyrveCIMonitor extends Disposable implements INyrveCIMonitor {
	...
	startMonitoring(owner: string, repo: string, branch: string): void {
		const ciEnabled = this.configurationService.getValue<boolean>('nyrve.github.ciMonitoring') ?? true;
		if (!ciEnabled) {
			return;
		}
		...
		this.logService.info(`[Nyrve] Started CI monitoring for ${owner}/${repo}@${branch}`);
	}
	...
	async fetchFailureLogs(owner: string, repo: string): Promise<string | undefined> {
		...
		try {
			...
			this.logService.warn(`[Nyrve] Failed to fetch CI logs: ${e}`);
			return undefined;
		}
	}
	...
	private async _poll(): Promise<void> {
		...
		try {
			...
			this.logService.warn(`[Nyrve] CI polling error: ${e}`);
		}
	}
...
registerSingleton(INyrveCIMonitor, NyrveCIMonitor, InstantiationType.Delayed);
