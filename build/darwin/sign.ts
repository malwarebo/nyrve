...
async function main(buildDir?: string): Promise<void> {
	...
	if (arch !== 'universal') {
		await spawn('plutil', [
			...
			'-string',
			'An application in Nyrve wants to use AppleScript.',
			`${infoPlistPath}`
		]);
		await spawn('plutil', [
			'-replace',
			...
			'-string',
			'An application in Nyrve wants to use the Microphone.',
			`${infoPlistPath}`
		]);
		await spawn('plutil', [
			'-replace',
			...
			'-string',
			'An application in Nyrve wants to use the Camera.',
			`${infoPlistPath}`
		]);
		await spawn('plutil', [
			'-replace',
			...
			'-string',
			'An application in Nyrve wants to use Audio Capture.',
			`${infoPlistPath}`
		]);
	}
