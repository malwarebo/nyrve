# Nyrve IDE — AI-Native Code Editor

## What Is This Project

Nyrve is a VS Code fork with Claude's agentic coding capabilities deeply integrated into every layer of the editor. Unlike Cursor (interactive AI editing) or Copilot (AI bolted onto an existing editor), Nyrve treats the AI agent as a first-class citizen — capable of autonomous multi-file edits, background monitoring, GitHub workflow automation, and persistent project memory.

**Full product specification:** `.claude/nyrve-spec.md`
...
Nyrve inherits VS Code's layered architecture and adds a `src/nyrve/` directory for all Nyrve-specific code. Never modify VS Code core files directly unless absolutely necessary — prefer extending via contribution points and service injection.

### Nyrve-Specific Code (`src/nyrve/`)

All new Nyrve code lives here. This is the code YOU are building:
...
├── ui/              # All Nyrve UI panels (agent chat, diff review, task queue, etc.)
...
### Root Folders
- `src/` — Main TypeScript source with unit tests in `src/vs/*/test/` and `src/nyrve/*/test/`
- `build/` — Build scripts and CI/CD tools
- `extensions/` — Built-in extensions that ship with VS Code
- `test/` — Integration tests and test infrastructure
- `.nyrve/` — Per-project Nyrve runtime data (index.db, memory.db, tasks.db, config.json)
- `.claude/` — Claude Code config and project specs

### Core Principles
- **Layered architecture** — from `base` → `platform` → `editor` → `workbench` → `nyrve`
...
- **Nyrve isolation** — All Nyrve code in `src/nyrve/`, never pollute VS Code core unless required for deep integration
- **Cross-platform** — Must work on macOS, Windows, and Linux

## Implementation Priorities

### Current Phase: Phase 1 — Foundation
Reference: `.claude/nyrve-spec.md` Section 9, Phase 1

1. ~~Fork VS Code and set up build pipeline with custom branding~~ (DONE if you're reading this)
2. Build `NyrveEditorBridge` — real-time editor state API (spec: FR-4.1.3)
...
Before building Nyrve features, disable/remove VS Code's built-in AI chat and Copilot integration so only the Nyrve Agent Panel is visible. Reference: spec Section 6.
...
- Preferred approach: create `src/nyrve/core/nyrve-disable-native-ai.ts` that deregisters all chat/copilot contribution points at startup (cleaner than deleting source files for upstream sync)

### Up Next: Phase 2 — Tier 1 Features
- Nyrve Settings Page — full graphical settings UI (spec: Section 7.1)
...
Nyrve is a Claude-exclusive IDE. All AI features are powered by Claude via the Anthropic API. No third-party AI providers (OpenAI, Google, etc.) are supported or planned.

- **Authentication:** Bring-your-own-key model. Users paste their Anthropic API key into Nyrve. All API calls go directly from the user's machine to api.anthropic.com. There is no Nyrve backend or proxy. Reference: spec Section 6.
...
- **Claude model family only:** Nyrve supports switching between all available Claude models (Opus, Sonnet, Haiku) — but never non-Claude models
...
- **No-key mode:** Without an API key, Nyrve works as a normal code editor. All AI features are inactive but the editor functions normally.
...
- Nyrve-specific tests: `scripts/test.sh --grep "Nyrve"` (all Nyrve tests should be prefixed with "Nyrve:")
- Layering validation: `npm run valid-layers-check`

### Nyrve-Specific Validation
...
- After team knowledge changes: verify suggestion flow (agent suggests → user approves → appended to .nyrve/team-knowledge.md)
...
### Nyrve-Specific Conventions
- All Nyrve services must be registered in the DI container following VS Code's pattern
- Nyrve UI panels use VS Code's webview API — they render inside the editor, not in a browser
- All Nyrve database operations (SQLite for index, memory, tasks) must be async and non-blocking
...
- All Nyrve settings must be namespaced under `nyrve.*` in the settings schema
- Nyrve config files live in `.nyrve/` at the project root
- Rust native addon code (indexer) lives in `src/nyrve/indexer/native/` with its own Cargo.toml
...
- Inline comments for complex Nyrve-specific logic (agent orchestration, context building, etc.)

### Tests
- All Nyrve tests go in `src/nyrve/*/test/` alongside the source
- Test names prefixed with `Nyrve:` for easy filtering
...
1. **Nyrve code first:** If implementing a Nyrve feature, start in `src/nyrve/`
2. **Check the spec:** Reference `.claude/nyrve-spec.md` for interfaces, requirements, and architecture
...
Nyrve tracks VS Code upstream releases. When syncing:
- All Nyrve code is in `src/nyrve/` — this should never conflict with upstream
- If upstream changes break Nyrve integration points, fix in `src/nyrve/` not in VS Code core
- Test full build and all Nyrve features after any upstream merge
